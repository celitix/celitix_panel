export const convertPaylaod = (data: string) => {
  ...
  parsedData.forEach((item, index) => {
    ...
    console.log("item.nodeIndex", item.nodeIndex);

    // Process conditions and edges
    const conditionList = item[id]?.conditionList || {};
    Object.entries(conditionList).forEach(
      ([key, condition]: [string, any], conditionIndex) => {
        const nodeType = condition?.nextNode?.split("_")[0];
        const nodeId = condition?.nextNode?.split("_")[1];
        const newId = `${nodeType}_${Number(nodeId) + 1}`;
        const targetNode = parsedData.find(
          (i) => idMap[i?.nodeId] === newId
        )?.nodeIndex;
