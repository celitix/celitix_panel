...
export const RCS = ({
  ...
  function handleSave() {
    ...
    Object.keys(inputVariables).map((key) => {
      const input = inputVariables[key];
      const formattedKey = input.replace(/^\{(#.+?#)\}$/, "$1");
      inputVar.push(formattedKey);
    });
    ...
    Object.keys(btninputVariables).map((key) => {
      const input = btninputVariables[key];
      const formattedKey = input.replace(/^\{(#.+?#)\}$/, "$1");
      btnVar.push(formattedKey);
    });
    ...
    const contentWithVar = content?.map((v, index) => ({
      [v]: inputVar[index],
    }));
    const btn = btnvarList?.map((v) => v.match(/{#(.+?)#}/)?.[1]);

    const contentWithBtnVar = btn?.map((v, index) => ({
      [v]: btnVar[index],
    }));

    variables = [...contentWithVar, ...contentWithBtnVar];

    // console.log("variables", variables);
    variables = variables.reduce((acc, obj) => {
      return { ...acc, ...obj };
    }, {});

    // //   variables = [...content, ...btn];
    //   console.log("variables12", variables);
