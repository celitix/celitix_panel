...
const LookUpTable = ({ id, name, data, onDataReady }) => {
  ...
  useEffect(() => {
    ...
    const rws = data?.map((row, idx) => ({
      sn: idx + 1,
      id: row.srNo || idx,
      insertTime: moment(row.insertTime).format("YYYY-MM-DD hh:mm:ss"),
      lookupDescription: row.lookupDescription,
      lookupStatus: row.lookupStatus,
      mobileNo: row.mobileNo,
      originalCircle: row.originalCircle,
      originalOperator: row.originalOperator,
      ported: row.ported,
      portedCircle: row.portedCircle,
      portedOperator: row.portedOperator,
      status: row.status,
    }));
    ...
    if (onDataReady) {
      onDataReady({ rows: rws, columns: cols });
    }
  }, [data]);
