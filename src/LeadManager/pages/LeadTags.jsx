...
import {
  ...
  getSingleTag,
  getTagsList,
} from "@/apis/LeadManagements/Tags/Tags";
...
export default function LeadTags() {
  ...
  const allChecked =
    filtered.length > 0 && filtered.every((t) => selectedIds.has(t.srNo));
  const toggleAll = () => {
    if (allChecked) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(filtered.map((t) => t.srNo)));
    }
  };
  ...
  const handleSaveTag = async () => {
    ...
    try {
      ...
      if (res?.success) {
        toast.success(
          res.message ||
          (form.srNo ? "Tag updated successfully" : "Tag added successfully")
        );
        fetchTags();
  ...
  const StatusToggle = ({ status, onToggle }) => {
    return (
      <button
        type="button"
        onClick={onToggle}
        className={`relative cursor-pointer inline-flex h-6 w-11 items-center rounded-full transition-all duration-200 ${status === 1 ? "bg-green-500" : "bg-gray-300"
          }`}
      >
        <span
          className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition-all duration-200 ${status === 1 ? "translate-x-6" : "translate-x-1"
            }`}
        />
      </button>
    );
  ...
  // handles API + state update for toggle
  const handleToggleStatus = async (tag) => {
    const newStatus = tag.status === 1 ? 0 : 1;
    try {
      const res = await addEditTag(tag.srNo, { ...tag, status: newStatus });
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-full bg-gray-100 min-h-screen w-full">
        <header className="px-6 py-4 bg-white border-b shadow-sm flex items-center justify-between">
          <h1 className="text-3xl font-semibold text-gray-800 playf">
            Tags &amp; Sources Manager
          </h1>
          <p className="mt-1 text-sm text-gray-900 font-medium">
            Organize, categorize, and manage tags efficiently. View statistics,
            edit details, and keep your workflow structured.
          </p>
        </header>
        ...
        <div className="bg-white px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between sm:space-x-4 ">
          <div className="flex items-center space-x-3 mb-3 sm:mb-0">
            <input
              ...
              className="flex-1 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-300 w-70"
            />
        ...
        <div className="overflow-scroll mb-22 w-full">
          <div className=" overflow-x-scroll scroll-smooth">
            <table className="w-full divide-y divide-gray-200 bg-white overflow-scroll">
              ...
              <tbody className="divide-y divide-gray-100">
                {tags.length === 0 ? (
                  ...
                  <tr className="h-full">
                    <td colSpan={9} className="py-56">
                      <div className="flex flex-col items-center justify-center text-center">
                        <div className="mb-4 animate-bounce">
                          <FaTags className="text-5xl text-blue-500" />
                        </div>
                        <h2 className="text-xl font-semibold text-gray-800">
                          No Tags Created Yet
                        </h2>
                        <p className="mt-2 text-sm text-gray-500 max-w-sm">
                          Tags help you organize your leads, candidates, and
                    ...
                    <td colSpan={9} className="py-56">
                      <div className="flex flex-col items-center justify-center text-center">
                        <div className="mb-4 animate-pulse">
                          <MdSearchOff className="text-6xl text-red-400" />
                        </div>
                        <h2 className="text-xl font-semibold text-gray-800">
                          No Matching Tags Found
                        </h2>
                        <p className="mt-2 text-sm text-gray-500 max-w-sm">
                          We couldn’t find any tags matching your search. Try a
                          different keyword or create a new tag instead.
                        </p>
                  ...
                  filteredTags.map((tag, index) => (
                    <tr key={tag.srNo} className="hover:bg-gray-50 transition">
                      ...
                      {/* Tag Name */}
                      <td className="px-4 py-3 text-sm font-medium text-gray-800 max-w-[150px] text-wrap break-words">
                        {highlightMatch(tag.tagName || "", filter)}
                      </td>

                      {/* Tag Details */}
                      <td className="px-4 py-3 text-sm text-gray-600 max-w-[200px] text-wrap break-words">
                        {tag.tagDetails}
                      </td>
                      ...
                      <CustomTooltip
                        ...
                        placement="top"
                        title={
                          tag.status === 1 ? "Tag is active" : "Tag is inactive"
                        }
                      >
                        <td className="px-4 py-3 text-center">
                          <StatusToggle
            ...
            <div className="flex flex-col items-start">
              ...
              <div className="flex items-start gap-3">
                <button
                  type="button"
                  onClick={() =>
                    handleChange("status", form.status === 1 ? 0 : 1)
                  }
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-500">
              ...
              candidates or records linked with this tag will lose their
              association.
            </p>
