...
export const Sidebar = ({ chatState, setChatState, isLoading, handleIsView
}) => {
  const {
    contextAgentList,
    rcsChatData,
  ...
  return (
    <div className="mt-2 pb-50 h-[100vh] max-h-full overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
      {!contextAgentList?.id && chatState.allConversations.length === 0 && (
        <motion.div
            ...
            chatState.allConversations
              ...
              ?.map((chat, index) => (
                <motion.div
                  ...
                  onClick={() => {
                    setChatState({ ...chatState, active: chat })
                    handleIsView();
                  }}
                >
