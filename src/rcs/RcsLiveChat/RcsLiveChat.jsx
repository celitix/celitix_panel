...
const RcsLiveChat = () => {
  ...
  const [shouldFetchOnce, setShouldFetchOnce] = useState(false);
  ...
  useEffect(() => {
    if (contextAgentList?.all?.length > 0) {
      setContextAgentList(prev => ({
        ...prev,
        id: prev.all[0].agent_id,
      }));
    }
  }, [contextAgentList.all]);
  ...
  async function handleFetchTemplates() {
    try {
      if (!contextAgentList?.id) return;
      const res = await fetchAllTemplates(contextAgentList?.id, "1", "approved");
      setTemplateState({
        all: res?.Data,
        selected: "",
      });
  ...
  async function handleIsView() {
    ...
    if (!chatState?.allConversations[0]?.srno) return;
    try {
      const data = {
        mobile: chatState?.active?.mobileNo,
        agentId: contextAgentList?.id,
        srno: chatState?.allConversations[0]?.srno,
      };

      const res = await readMessageRcs(data);
      if (res?.status === true) {
        setShouldFetchOnce(true);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  useEffect(() => {
    ...
    handleIsView();
  }, [chatState?.allConversations[0]?.srno]);
