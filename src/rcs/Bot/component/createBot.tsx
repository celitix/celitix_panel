...
import rcsicon from "@/assets/icons/rcs.png";
...
import { formatNodes } from "./components/formatNodes";
...
const CreateRcsBot = () => {
  ...
  // }
  if (state?.data) {
    const body = JSON.parse(state?.data?.jsonbody);
    const { nodes, edges } = convertToReactFlow(body);
    const options = transformNodesById(body);
    ...
    async function handleFetchAgent() {
      try {
        ...
        setDetails((prev) => ({
          ...prev,
          rcs: res || [],
          selected: state?.data?.rcsAgentId || "",
          name: !state?.isDuplicate ? state?.data?.botName : "",
        }));
  ...
  const onConnect = useCallback(
    (connection: { source: any; target: any }) => {
      ...
      const edgeType = { ...connection, type: "deletable" };
      setEdges((eds) => addEdge(edgeType, eds));
      setNonAnswerCount((prev) => prev - 1);
    },
    [edges, setEdges]
  );
  ...
  //   if (prevNode?.type == "location" && type != "answer") {
  //     return toast.error("You can add answer node only after location node");
  //   }
  //   const newNode = {
  //     id: `${nodeId}`,
  ...
  //   // const nonAnswerNode = nodes.filter((node) => node.type !== "answer");
  //   let prevNodeL = nodes?.length

  //   if (prevNodeL != 0 && prevNodeL % 2 == 0 && type !== "answer") {
  //     toast.error("You can add answer node only after location node");
  //     return
  //   }

  //   setNodes((prevNodes) => [...prevNodes, newNode]);

  //   let isAnswerNodeInsert = false
  ...
  const addNode = (type: string, position?: { x: number; y: number }) => {
    // RULE: If 2 non-answer nodes already added, force answer node next
    if (nonAnswerCount === 2 && type !== "answer") {
      return toast.error(
        "An answer node is required after two non-answer nodes."
      );
    }
    ...
    // Update non-answer streak
    if (type === "answer") {
      setNonAnswerCount(0); // reset
    } else if (type == "starting") {
      setNonAnswerCount(0);
    } else {
      setNonAnswerCount((prev) => prev + 1); // count non-answer nodes
    }

    // Increment id
    setNodeId((prev) => prev + 1);

    // Auto position next node
    if (!position) {
  ...
  const deleteNode = useCallback(
    (nodeId: string) => {
      ...
      setIsVisible(false);
      setNonAnswerCount((prev) => prev - 1); // reset
    },
  ...
  function handleSaveNodeData() {
    ...
    if (type === "template" && !nodeData.templateName) {
  ...
  const handleSubmit = async (isClose = true) => {
    ...
    // }
    const srno = details?.rcs.find(
      (item) => item.agent_id === details?.selected
    )?.srno;

    const botDetails = {
      botName: details?.name,
      rcsAgentId: details?.selected,
      rcsAgentSrNo: srno,
      deptName: name,
    };
    ...
    try {
      ...
      const botSrno = state?.isDuplicate ? "" : state?.data?.botSrno;
      const res = await saveOrEditBot(payload, botSrno);
      if (!res?.status) {
  ...
  function handleFormat() {
    const { node, edge } = formatNodes(nodes, edges, "LR", {
      nodesep: 80,
      ranksep: 100,
    });
    setNodes(node);
    ...
    <>
          ...
          >
            ...
            <div className="flex gap-5 items-center">
              <h1 className="text-lg font-semibold text-indigo-900 tracking-tight">
                Design Your RCS Automation Bot
              </h1>
              {/* <motion.button
                whileHover={{ scale: 1.1, rotate: 5 }}
                whileTap={{ scale: 0.9, rotate: -5 }}
                onClick={handleFormat}
                className="p-2 rounded-full bg-indigo-100 text-indigo-600 hover:bg-indigo-200 shadow-md transition-colors"
              >
                <FaShuffle className="text-lg" />
              </motion.button> */}
              <CustomTooltip
                title={
                  <div className="max-w-xs p-2 bg-white rounded-xl shadow-md text-center">
                    <p className="font-medium text-indigo-700">Format Bot</p>
                    <p className="text-gray-600 text-sm mt-1">
                      Automatically align and organize your canvas nodes for a
              ...
              >
                <motion.button
                  ...
                  onClick={handleFormat}
                  className="p-2 rounded-full bg-gradient-to-r from-green-500 to-green-500 text-white shadow-lg cursor-pointer border-2 border-indigo-400"
                >
                  <FaShuffle className="text-lg" />
                </motion.button>
              </CustomTooltip>
            ...
            <div className="flex flex-col sm:flex-col md:flex-row items-center gap-3 justify-between relative ">
              ...
              >
                <Details
                  ...
                  isUpdate={state?.isDuplicate ? false : true}
                  ...
                  <div className="p-6 sm:p-8 bg-white rounded-b-2xl">
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50">
                        <div className="h-auto w-7 overflow-hidden">
                          <img src={rcsicon} alt="" />
                        </div>
                      </div>
                      ...
                      <div className="flex-1">
                        <h2 className="text-xl font-semibold text-gray-900">
                          Select a RCS BOT to get started
                        </h2>
                        <p className="mt-1 text-sm text-gray-600">
                          Choose a RCS from the selector above to initialize
                    ...
                    <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                      <div className="flex items-start gap-3 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-600">Bind RCS</p>
                        </div>
                      </div>
                    ...
                    <div className="mt-5 rounded-lg border border-dashed border-gray-300 bg-white/70 px-3 py-2 text-center">
                      <p className="text-xs text-gray-600">
                        Tip: Select a RCS from the dropdown to unlock the canvas
                        and start building your RCS bot.
                      </p>
                    </div>
                  </div>
                </motion.div>
        ...
        <div className="w-auto gap-4 rounded-md h-auto mt-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            {/* <Button
              ...
              className={commonButtonClass}
            >
              <GrLocation className="text-purple-900 size-6" />
            ...
            >
              ...
              >
                ...
                <span className="text-xs font-semibold">GoTo</span>
              </Button>
            </motion.div> */}
      ...
      <Dialog
        header={
          <>
            ...
            {type === "text" ? (
              ...
              <p className="text-sm">
                (Make Location options for the Location node)
              </p>
            ) : null}
          </>
        }
        ...
        onHide={() => {
          ...
          className="lg:w-[60rem] md:w-[20rem] w-[35rem]"
        draggable={false}
      >
