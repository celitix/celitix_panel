function generateBotPayload(
  ...
  orderedNodes.forEach((node, index) => {
    ...
    if (finalType === "START") {
      ...
      entry["rcsAgentId"] = botDetails?.rcsAgentId;
      entry["rcsAgentSrNo"] = botDetails?.rcsAgentSrNo?.toString();
      entry["startKeyword"] = nodeInput?.startingKeyword;
      entry["channelType"] = "RCS";
    }
    if (finalType === "text") {
    ...
    if (finalType === "template") {
      entry["customParams"] = nodeInput?.customParams;
      entry["templateName"] = nodeInput?.templateName;
    }
    ...
    if (finalType === "location") {
      entry["textMessage"] = nodeInput?.textMessage;
    }
