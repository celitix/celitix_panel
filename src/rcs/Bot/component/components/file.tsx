...
export const FileNodeContent = ({
  ...
  function updateCaption(newVal: string) {
    setNodesInputData((prev) => ({
      ...prev,
      [id]: { ...prev[id], fileCaption: newVal },
    }));
  }
  const getCaption = () => nodesInputData[id]?.fileCaption ?? "";

  function addFormat(formatType: "bold" | "italic" | "strike" | "list") {
    const el = inputRef.current;
    if (!el) return;

    const input = getCaption();
    ...
    if (formatType !== "list") {
      ...
      requestAnimationFrame(() => {
        // place caret just after the closing token (preserves selection collapse)
    ...
    if (hasSelection) {
      // transform each non-empty selected line to "- ..."
      const selected = input.slice(selectionStart, selectionEnd);
      ...
      // insert a new bullet at caret and enter listMode
      const insert = "- ";
      const newValue =
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    // Identify the current line boundaries
    ...
    // Remove bullet with Backspace at start of "- "
    if (e.key === "Backspace") {
      const caretInFirstTwo = selectionStart - lineStart <= 2; // at '-', ' ', or start
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-1 gap-2">
        ...
        <div className="flex items-center justify-center rounded-lg border w-full h-70 mt-7">
          {nodesInputData[id]?.fileUrl &&
            (accept === "image" ? (
              <img
                src={
