...
const Bot = () => {
  ...
  const [dropdownItemsHidden, setDropdownItemsHidden] = useState([
    // "Edit",
    // "Duplicate",
    "Export",
  ]);
  ...
  async function handleFetchAllBot() {
    const channelType = "RCS"
    try {
      setIsLoading(true);
      const res = await getAllBot(channelType);
  ...
  async function handledeleteBot(id) {
    ...
    try {
      ...
      if (res?.status) {
        ...
        toast.error(res?.msg || "Failed to delete bot!");
      }
    } catch (e) {
  ...
  const handleExportJson = (bot) => {
    try {
      // Parse the stringified JSON
      const jsonData = JSON.parse(bot.jsonbody);
      ...
      link.download = `${bot.botName || "bot"}.json`;
      link.click();
    } catch (err) {
  ...
  async function handleBtnClick(item, data) {
    if (item === "Edit" || item === "Duplicate") {
      // navigate("/rcsbot", {
      navigate("/rcs/create-bot", {
        state: {
          data,
  ...
  const rowsPerPage = 4;
  const filteredBots = (Array.isArray(allBots) ? allBots : []).filter((bot) => {
      ...
      {isLoading ? (
        ...
        <div className="p-3 rounded-xl space-y-6 bg-gray-50  w-full overflow-hidden min-h-[90vh]">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 md:grid-cols-3 items-center mb-4">
            {/* Title */}
            <div className="flex items-center justify-center lg:justify-start gap-2 font-semibold text-gray-900">
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between items-center mb-4 bg-white">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2 mb-2 sm:mb-0">
                  ...
                  <div className="relative inline-block" ref={dropdownRef}>
                    {/* <div
                      className="text-white opacity-0 select-none"
                      onClick={() => setIsOpen(!isOpen)}
                    >
                      a
                    </div> */}
                    {isOpen && (
                      <div className="absolute left-0 mt-1 w-64 max-h-40 overflow-y-auto bg-white border rounded-md shadow-lg">
                        {allBots.map((bot, index) => (
                          <div
                            key={bot.botSrno}
                            className="flex justify-between items-center px-3 py-2 border-b last:border-b-0 hover:bg-gray-50"
                          >
                            <div className="text-[0.8rem] font-medium text-gray-800">
                              {highlightMatch(
                                String(bot.botName || ""),
                                search
                              )}
                            </div>
                            {dropdownItemsHidden.map((item, idx) => {
                              const iconMap = {
                                Export: (
                              ...
                              return (
                                <button
                                  ...
                                  className="text-xs text-slate-600 cursor-pointer hover:text-slate-800"
                                >
                                  {iconMap[item]}
                                </button>
                              );
                          ...
                          <div className="grid lg:grid-cols-4 xl:grid-cols-5 bg-blue-100 px-6 py-4   xs:grid-cols-1 md:grid-cols-3 gap-2 sm:grid-cols-3  items-center justify-between flex-wrap sm:flex-nowrap">
                            <div className="flex items-center gap-4 flex-1">
                              ...
                              <div className=" text-center ">
                                ...
                                <div className="text-[0.92rem] font-semibold text-gray-800">
                                  {" "}
                                  {highlightMatch(
                                    String(bot.botName || ""),
                                    search
                            ...
                            <div className="flex items-center justify-end gap-2 relative">
                              <CustomTooltip title="Settings" arrow>
                                <IconButton
                                  ref={(el) => {
                                    if (el)
                                      dropdownButtonRefs.current[bot.botSrno] =
                                        el;
                                  }}
                                  ...
                                  {dropdownItems.map((item, idx) => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className="w-full text-left px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-md flex items-center gap-2 cursor-pointer"
                                      >
                                        {iconMap[item]}
                                        {item}
                                      </button>
        ...
        <div>
          <div className="p-4 text-center">
            ...
            <div className="flex justify-center gap-4 mt-2">
              ...
              <UniversalButton
                // label="Delete"
                label={isDeleting ? "Deleting Bot..." : "Delete"}
                disabled={isDeleting}
                style={{
                  backgroundColor: "red",
                }}
