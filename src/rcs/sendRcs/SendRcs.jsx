...
const SendRcs = () => {
  ...
  const [isFinalSend, setIsFinalSend] = useState(false)
  ...
  async function handleFinalSend() {
    ...
    try {
      ...
      if (res?.status === "success" || res?.statusCode === 200) {
        ...
        toast.success("Campaign Launch Successfully");
        // setAllAgents([])
        setAllTemplates([]);
  ...
  async function handleFetchWorkflow() {
    try {
      const res = await getAllWorkflow("rcs");
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-2 md:grid-cols-3 my-5">
      ...
      >
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-center">
            <UniversalButton
              // label="Send Campaign"
              label={isFinalSend ? "Sending Campaign..." : "Send Campaign"}
              disabled={isFinalSend}
