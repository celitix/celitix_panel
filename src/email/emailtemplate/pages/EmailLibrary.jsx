...
import { getEmailLibraryData } from "@/apis/email/Email";
...
const predefinedTemplates = Array.from({ length: 20 }, (_, i) => ({
  ...
  description:
    "This is a beautifully crafted email template to engage your users effectively.",
  icon:
    i % 2 === 0 ? (
...
const EmailLibrary = () => {
  ...
  const [pageSize] = useState(12); // ✅ ADD
  ...
  const fetchLibrary = async () => {
    try {
      ...
      const payload = {
        ...
        pageSize: pageSize,
        templateType: selectedType === "All" ? "" : selectedType,
      };

      console.log("Payload:", payload);

      const res = await getEmailLibraryData(payload);
        ...
        {filteredTemplates.map((template) => (
          ...
          >
            <div className="absolute inset-0 backdrop-blur-sm opacity-0.5 group-hover:opacity-100 transition-opacity duration-300 z-10 flex flex-col items-center justify-center gap-3 px-4 hover:border-2 rounded-2xl border-blue-800">
              <div className="px-4 py-1.5 flex items-center gap-3 rounded-full bg-[#578FCA] text-white text-sm font-medium tracking-wider shadow-md hover:bg-[#3674B5] transition-transform transform hover:scale-105">
                <MdOutlineZoomOutMap fontSize="20px" /> Preview
              </div>
              <div className="px-4 py-1.5 rounded-full bg-green-500 text-white text-sm font-medium shadow-md hover:bg-green-600 flex items-center gap-3 transition-transform transform hover:scale-105 tracking-wider">
            ...
            <div className="flex flex-col items-center text-center relative z-0">
              <div
                className={`rounded-full w-14 h-14 flex items-center justify-center mb-3 shadow-inner ${template.type === "Transactional" ? "bg-blue-500" : "bg-green-500"}`}
              >
                {template.icon}
              </div>
              ...
              <div
                className={`text-xs mt-2 mb-3 px-3 py-1 rounded-full font-medium ${template.type === "Transactional" ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"}`}
              >
                {template.type}
              </div>
