...
// APIS
import { emailLibraryAddorUpdate } from "@/apis/email/Email";
import { uploadImageFile } from "@/apis/whatsapp/whatsapp.js";
import { utilityFileUpload } from "@/apis/utility/Utility";

// COMPONENTS
import Preview from "../../emailtemplate/components/Preview";
...
const AddTemplate = () => {
    ...
    const emailTemplate = location.state?.emailTemplateData;
    ...
    useEffect(() => {
        if (emailTemplate) {
            setForm({
                name: emailTemplate?.templateName
            })
            setText(emailTemplate?.emailContent)
        }
    }, [emailTemplate])
    ...
    const handleFileChange = async ({ files }) => {
        ...
        if (file) {
            ...
            let generateHandler = 1

            try {
                // const response = await uploadImageFile(file);
                const response = await utilityFileUpload(file, generateHandler); // your API call
    ...
    const handleSave = async () => {
        ...
        const payload = {
            name: form.name,
            emailBody: text,
            // attachmentId: uploadedFile?.url || null,
            // variableCount: count,
            subject: form.subject,
            category: form.category,
        };

        // Normalize srNo (avoid 0)
        const srNo =
            emailTemplate?.srNo && emailTemplate.srNo > 0
                ? emailTemplate.srNo
                : null;
        ...
        try {
            if (srNo) {
                // UPDATE
                response = await emailLibraryAddorUpdate(payload, srNo);
            } else {
                // CREATE
                response = await emailLibraryAddorUpdate(payload);
            }
                    ...
                    >
                        <div>
                            ...
                            <input
                                type="text"
                                name="name"
                                value={form.name || emailTemplate?.templateName}
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap xl:flex-nowrap gap-5 space-y-2 xl:space-y-0">
                                {categoryOptions.map((option) => (
                                    <motion.label
                                        key={option.id}
                                        className={`relative flex items-center cursor-pointer select-none px-1.5 py-0.5 rounded-lg transition-all duration-300
