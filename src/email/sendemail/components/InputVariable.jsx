...
const InputVariable = ({ variables = [], onSelect }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        className="px-1 border-1 border-gray-300 rounded-r-sm h-9 text-sm bg-gray-200 cursor-pointer focus:outline-none"
        onClick={() => setShowDropdown(!showDropdown)}
      >
        <PiBracketsCurlyBold />
      </button>
      {showDropdown && (
        <div className="absolute min-w-30 z-99999  md:right-[25px] right-[25px] max-h-47 overflow-auto  bg-white border border-gray-300 rounded-sm shadow-md bottom-0 ">
