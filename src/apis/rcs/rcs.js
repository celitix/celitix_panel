...
export const readMessageRcs = async (data) => {
  return await fetchWithAuth(
    `/rcs/isRcsviewUpdate?srno=${data.srno}&agentId=${data.agentId}&mobile=${data.mobile}`,
    {
      method: "POST",
    },
  );
...
// export suggestion data
export const changeRcsLiveStatus = async (data) => {
  return await fetchWithAuth("/rcs/changeLiveStatus", {
    method: "POST",
    body: JSON.stringify(data),
  });
};

// insert rcs callback url with waba
...
// get rcs callback url with waba
