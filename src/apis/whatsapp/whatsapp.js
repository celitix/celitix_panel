...
// get waba display name status
export const getWabaDisplayNameStatus = async (wabaSrno) => {
  return await fetchWithAuth(
    `/whatsapptemplate/getBusinessTemplateDetails?wabaSrno=${wabaSrno}`,
    {
      method: "POST",
    },
  );
...
// /whatsapptemplate/getBusinessTemplateDetails?wabaSrno=2 - POST  - getWabaBusinessTempDetails
...
// Get All Bot
export const getAllBot = async (channelType) => {
  return await fetchWithAuth(`/bot/getBotFlowList?channelType=${channelType}`, {
    method: "POST",
  });
};
...
// call settings
export const saveCallSettingData = async (data) => {
  return await fetchWithAuth(`/dummy/dummy`, {
    method: "POST",
    body: JSON.stringify(data),
  });
};
