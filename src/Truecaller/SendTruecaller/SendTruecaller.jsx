...
// COMPONENTS 
import { GridTruecaller } from "./components/GridTruecaller";
import { RadioButtonLaunchTruecaller } from "./components/RadioButtonLaunchTruecaller";
import UniversalButton from "@/components/common/UniversalButton";
import { PreviewTruecaller } from "./components/PreviewTruecaller";

// HELPER 
import { isEnglish } from "./helper/isEnglish";

const SendTruecaller = () => {
  ...
  function handleReview() {
    ...
    if (isError) return;
    const tag = `{#${inputDetails?.attachmentType}#}`;
    const value = inputDetails?.attachmentVar?.[tag];

    // console.log("value",value)
    // console.log("attachmentVar", inputDetails?.attachmentVar);
    // return;
    if (
      ...
      return toast.error(
        "Please enter valid short url. URL must starts with https"
      );
    }

    if (
      inputDetails?.attachmentType === "whatsapp_chat" &&
      !/(?:https?:\/\/)?wa\.me\/[0-9]+/.test(value)
    ) {
      return toast.error(
        "Please enter valid whatsapp URL. URL must starts with https and include wa.me"
      );
    }

    if (inputDetails?.attachmentType === "file" && !value) {
      return toast.error("Please upload a file.");
    }
    ...
    <>
      <div className="flex gap-2 w-full mt-3 flex-col lg:flex-row">
        ...
        <RadioButtonLaunchTruecaller
          ...
          inputRef={inputRef}
        />
        <PreviewTruecaller inputDetails={inputDetails} />
        {/* </div> */}
      </div>
      ...
      >
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="scheduleCheckbox" className="text-md text-gray-700">
              Schedule
            </label>
