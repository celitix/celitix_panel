...
const WhatsappManageCampaign = () => {
    ...
    const fetchWabaList = async () => {
      try {
        ...
        if (Array.isArray(response) && response.length > 0) {
          ...
          setSelectedWaBaNumber(response[0].wabaSrno);
        } else {
  ...
  const handleTableDataReady = ({ rows, columns }) => {
    setTableData(rows);
  ...
  const handleExportSummary = () => {
    ...
    const col = tableColumns.map((c) => c.field);
    const row = tableData.map((r) => col.map((f) => r[f] ?? ""));
    exportToExcel(col, row, "Summary Report");
    toast.success("File Downloaded Successfully");
  };
  ...
  const fetchWabaFlows = async (item) => {
    try {
  ...
  const fetchFlowMainResponse = async (item) => {
    ...
    const matched = flowList.find((flow) => flow.flowId === item.flow_id);

    if (!matched) {
      toast.error("No matched JSON found");
    }
    ...
    try {
      ...
      if (matched) {
        navigate("/wflowsdetailsreport", {
          state: { data: response, matchedFlow: matched },
        });
      }
    } catch (error) {
              ...
              {isFetching ? (
                ...
                <div className="w-full">
                  <ManageSummaryTable
                    ...
                    onDataReady={handleTableDataReady}
                  />
                </div>
              )}
            </div>
