...
const ManageCampaignTable = ({ id, name, data = [], fromDate }) => {
  ...
  //   const handleCtaAnalytics = async (row) => {
  //   setCtaAnalyticsLoading(true); // show loader
  //   setSelectedRow(row);
  //   setVisible(true);
  ...
  //         );
  //         if (wabaAccount)
  //           await fetchTemplateData(wabaAccount.wabaAccountId, row.templateName);
  //       })(),
  //     ]);
  //   } catch (error) {
  //     console.error("Error loading CTA Analytics:", error);
  ...
  const columns = [
    { field: "sn", headerName: "S.No", flex: 0, minWidth: 80 },
    {
      ...
      minWidth: 120,
      renderCell: (params) =>
        moment(params.row.queTime).format("DD-MM-YYYY HH:mm:ss"),
    },
    {
      field: "campaignName",
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div>
          ...
          >
            <IconButton
              onClick={() => reportListAction(params.row)}
            >
              <AiOutlineInfo
                sx={{
                  color: "gray",
