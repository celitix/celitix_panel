      ...
      if (res?.data) {
        ...
        const formattedData = res.data.map((item, index) => {
          ...
          return {
            sn: (page - 1) * paginationModel.pageSize + index + 1,
            id: `${item.id || "row"}-${
              (page - 1) * paginationModel.pageSize + index
            }`,
            wabaNumber: item.wabaNumber || "N/A",
            mobileNo: item.mobileNo || "N/A",
            source: item.source || "N/A",
  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <Paper
          sx={{ padding: 0, overflowX: "scroll", height: 607 }}
          ...
          <table className="w-full table-auto border-collapse overflow-x-scroll">
            <thead className="bg-[#e6f4ff] text-gray-700 text-sm">
              <tr>
                {[
                  ...
                  <th
                    key={i}
                    className="px-3 py-4 text-left border-b font-[500] border-r-2 text-[#193cb8]"
                  >
                    {header}
                  </th>
                ))}
            ...
            <tbody>
              {data.map((row, index) => (
                <tr
                  key={row.id}
                  className="border-b hover:bg-gray-50 text-sm w-full"
                >
                  ...
                  <td className="px-3 py-2 w-35 truncate text-wrap break-words text-start">
                    {row.reason}
                  </td>
                  ...
                  {/* <td className="px-3 py-2 max-w-xs text-wrap">{row.requestJson}</td> */}
                  ...
                  <td className="px-3 py-2 w-10">
                    <CustomTooltip title="Info" placement="top" arrow>
                      <span>
                        <IconButton
                          ref={(el) =>
                            (dropdownButtonRefs.current[row.id] = el)
                          }
