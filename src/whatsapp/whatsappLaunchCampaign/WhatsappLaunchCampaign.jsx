...
import { getPdfAllTemplate } from "@/apis/utility/Utility.js";
...
const WhatsappLaunchCampaign = () => {
  ...
  const [pdfMode, setPdfMode] = useState("upload"); // "dynamic" | "upload"
  const [pdfTempOptions, setPdfTempOptions] = useState([]);
  const [selectedPdfTemp, setSelectedPdfTemp] = useState(null);
  ...
  const fetchPdfTempList = async () => {
    ...
    try {
      ...
      //   ? response.filter(item => item.status === 1) // Keep only active
      //   : [];
      const list = Array.isArray(response)
        ? response
          .filter(item => item.status === 1) // Keep only active
          .sort((a, b) => new Date(b.insertTime) - new Date(a.insertTime)) // Sort newest first
        : [];
      setPdfTempOptions(list);
    } catch (error) {
  ...
  async function handleFetchWorkflow() {
    try {
      ...
      const whatsappNode = res?.filter((item) => item.node_type === "whatsapp");

      const mappedData = Array.isArray(whatsappNode)
        ? whatsappNode?.map((item) => ({
          value: item.sr_no,
          label: item.workflow_name,
        }))
        : [];
      setAllWorkflows(whatsappNode);
    } catch (e) {
  ...
  const handleSubmitCampaign = async () => {
    ...
    if (selectedTemplateData?.type === "document" &&
      pdfMode === "dynamic" &&
      !selectedPdfTemp) {
      toast.error("Please Select PDF Template or choose upload File option")
      return
    }
    ...
    //   const pdfSelected = pdfTempOptions.find(
    //     item => item.srNo === selectedPdfTemp
    //   );

    //   const vars = extractPdfVariables(pdfSelected.templateContent);

    //   for (let v of vars) {
    //     // if (!pdfVariables[v] || pdfVariables[v].trim() === "") {
    //     //   toast.error(`Please enter value for in PDF Variable Parameters #${v}#`);
    //     //   return;
    //     // }
    //     pdfFinal[v] = pdfVariables[v];
    //   }
    // }
    // setFinalPdfVars(pdfFinal);
  ...
  const handleFinalSubmit = async (event) => {
    ...
    // const buttonContentValues = Object.keys(inputValues)
    //   ?.map((variable) => {
    //     const key = `button${variable}`;
    //     const value = (formData[key] || "").trim();
    ...
    const buttonContentValues = Object.keys(inputValues);
    // console.log("inputValues", inputValues[buttonContentValues[0]]);
    // return;
    ...
    let requestData = {
      ...
      // urlValues: inputValues[buttonContentValues[0]],
      urlValues: inputValues[buttonContentValues[0]]
        ?.replace(/\{\{(.*?)\}\}/g, "#$1#"),
      urlIndex: urlIndex ?? -1,
      // isShortUrl: 1,
      isShortUrl: selectedTemplateData?.shortUrl || 0,
      isGroup: isGroup,
      ...
      cardsVariables: [],
      // vendor: "jio",
      vendor: selectedWabaData?.vendor,
      // // pdfMode,
      // dynamicPdfSrno: selectedPdfTemp || null,
      // dyMappingJson: pdfVariables,
    };

    if (selectedTemplateData?.shortUrl) {
      requestData = {
        ...requestData,
        url: "",
        urlValues: "short",
    ...
    if (selectedTemplateData?.type === "document") {
      requestData = {
        ...
        ...(pdfMode === "dynamic" && {
          isDynamicPdf: 1,
          dynamicPdfSrno: selectedPdfTemp,
          // dyMappingJson: finalPdfVars,
          dyMappingJson: pdfVariables,
        }),
      };
    }
    ...
    try {
      ...
      const isSuccess =
        ...
        response?.msg?.toLowerCase().includes("flow sent successfully");
      if (isSuccess) {
        ...
        <>
          <div className="container-fluid">
            <div className="flex flex-wrap">
              <div className=" w-full lg:w-2/3 p-3 rounded-xl flex lg:flex-nowrap flex-wrap gap-3.5 bg-gray-200 min-h-[80vh]">
                <div className="w-full lg:w-100 p-3 bg-gray-100 rounded-lg shadow-md lg:flex-1 ">
                    ...
                    <div className="mb-5 space-y-4">
                      ...
                      <div className="flex gap-6">
                        <label className="flex items-center gap-2">
                          <input
                            ...
                            onChange={() => setPdfMode("dynamic")}
                          />
                          <span>Select Dynamic PDF</span>
                        </label>
                      ...
                      {pdfMode === "dynamic" && (
                        <DropdownWithSearch
                          ...
                          label="Select Dynamic PDF"
                          tooltipContent="Select your PDF Template"
                          tooltipPlacement="right"
                          ...
                          onChange={(value) => {
                            ...
                            setPdfVariables({}); // reset on change
                          }}
                    ...
                    {isFetching ? (
                      ...
                      templateDataNew && (
                        <TemplateForm
                          ...
                          selectedPdfTemp={selectedPdfTemp}
                          pdfTempOptions={pdfTempOptions}
                          pdfVariables={pdfVariables}
                          setPdfVariables={setPdfVariables}
                          pdfMode={pdfMode}
                          setPdfMode={setPdfMode}
                          setSelectedPdfTemp={setSelectedPdfTemp}
                        />
                      )
                    )}
                  </div>
