...
import InputVariableForPdfInput from "./InputVariableForPdfInput";
...
const TemplateForm = ({
  ...
  const resetDynamicPdfSettings = () => {
    setSelectedPdfTemp(null);
    setPdfVariables({});
  ...
  // const extractPdfVariables = (content) => {
  //   const regex = /#(.*?)#/g;
  //   const matches = new Set();
  //   let m;

  //   while ((m = regex.exec(content)) !== null) {
  //     matches.add(m[1]); // extract only username, email, otp
  //   }

  //   return [...matches];
  // };

  // const extractPdfVariables = (html) => {
  ...
  const extractPdfVariables = (html) => {
    ...
    // Matches only non-color-code variable names like #username#
    const regex = /#(?![0-9A-Fa-f]{3}(?:[0-9A-Fa-f]{3})?$)([A-Za-z0-9_]+)#/g;
  ...
  const handleSaveDynamicPdf = () => {
    if (!selectedPdfTemp) {
      toast.error("Please select a PDF template.");
      return;
    }

    const selected = pdfTempOptions.find(t => t.srNo === selectedPdfTemp);
    const vars = extractPdfVariables(selected.templateContent);
    // console.log(vars)

    let tempVars = {};

    for (let v of vars) {
      if (!pdfVariables[v] || pdfVariables[v].trim() === "") {
        toast.error(`Please enter value for #${v}#`);
        return;
      }
      tempVars[v] = pdfVariables[v];
    }

    // SAVE TO GLOBAL STATE FOR FINAL SUBMIT
    setPdfVariables(tempVars);
    setShowPdfDialog(false);
    toast.success("Dynamic PDF settings saved!");
  ...
  return (
    <div className="rounded-md shadow-sm border border-[#128c7e42]">
      ...
      <div className="p-2 space-y-2 bg-gray-50 rounded-b-xl">
        ...
        {templateDataNew?.components.map((component, idx) => {
          if (component.type === "BUTTONS" && !tempData?.shortUrl) {
            return (
                    ...
                    <div key={index} className="space-y-4">
                      {urlVariables.map((variable) => (
                        ...
                        >
                          ...
                          <div className="absolute top-[27px] right-0 z-50">
                            <InputVariable
                              onSelect={(selectedVar) =>
        ...
        {selectedTemplateData?.type === "document" && (
          <div className="mb-2 p-2 rounded-xl border border-gray-200 bg-white shadow-sm space-y-5">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Choose how you want to generate your PDF file for this template.
              </p>
            </div>
            ...
            <div className="flex flex-wrap lg:flex-nowrap gap-2 pt-1">
              ...
              <label
                className={`
          flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition w-full
          border ${pdfMode === "dynamic" ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:bg-gray-100"}
          `}
              >
                <input
                  type="radio"
                  name="pdfMode"
                  value="dynamic"
                  checked={pdfMode === "dynamic"}
                  onChange={() => {
                    resetDynamicPdfSettings();
                    setPdfMode("dynamic");
                  }}
                  className="h-4 w-4"
                />
                <span className="text-sm text-gray-700 font-medium">Use Dynamic PDF</span>
              </label>
              ...
              <label
                className={`
          flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition w-full
          border ${pdfMode === "upload" ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:bg-gray-100"}
          `}
              >
                <input
                  type="radio"
                  ...
                  className="h-4 w-4"
                />
                <span className="text-sm text-gray-700 font-medium">Upload PDF File</span>
              </label>

            </div>

            {/* Dynamic PDF Template Dropdown */}
            {/* {pdfMode === "dynamic" && (
              <div className="pt-1">
                <DropdownWithSearch
                  id="pdftemplatelist"
                  name="pdftemplatelist"
                  label="Choose Dynamic PDF Template"
                  tooltipContent="Select a template that supports dynamic PDF rendering."
                  tooltipPlacement="right"
                  options={pdfTempOptions?.map((item) => ({
                    label: item.templateName,
                    value: item.srNo,
                  }))}
                  value={selectedPdfTemp}
                  onChange={(value) => {
                    setSelectedPdfTemp(value);
                    setPdfVariables({});
                  }}
        ...
        <Dialog
          header="Dynamic PDF Settings"
          visible={showPdfDialog}
          onHide={() => setShowPdfDialog(false)}
          className="lg:w-[80rem] md:w-[30rem] w-[20rem]"
          draggable={false}
        >
          <div>
            <DropdownWithSearch
            ...
            {selectedPdfTemp && (
              <div className="mt-2 space-y-4">
                ...
                {(() => {
                  const pdfTemplate = pdfTempOptions?.find(
                    (item) => item.srNo === selectedPdfTemp
                  );
                  if (!pdfTemplate) return null;

                  const pdfVars = extractPdfVariables(pdfTemplate.templateContent);

                  return (
                    <div className="flex gap-4">
                      <div className="border p-2 rounded-md mb-2 w-1/2">
                        {/* TITLE */}
                        <div className="text-center border-b pb-2 mb-2">
                          <h3 className="text-sm font-semibold text-gray-700">
                            PDF Variable Parameters ({pdfVars.length} Required Fields)
                          </h3>
                          <p className="text-xs text-gray-500 mt-1">
                            Please provide values for all required PDF variables below.
                            These details will be dynamically inserted into the PDF template.
                          </p>
                        </div>

                        <div className="max-h-100 overflow-auto z-0">
                          {/* INPUTS FOR EACH PDF VARIABLE */}
                          {pdfVars.map((v) => (
                            <div key={v} className="relative flex flex-col mb-3">
                              <InputField
                                label={`#${v}#`}
                                type="text"
                                placeholder={`Enter value for #${v}#`}
                                value={pdfVariables[v] || ""}
                                onChange={(e) =>
                      ...
                      {/* PREVIEW */}
                      <div className="border rounded-md bg-gray-50 w-1/2">
                        <p className="font-medium text-gray-700 my-2 pb-2 text-sm text-center border-b">{pdfTemplate.templateName} -  Preview</p>
                        <div className="p-1.5">
                          <div className="max-h-100 overflow-auto" >
                            <PdfFormatter htmlContent={pdfTemplate.templateContent} />
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          <div className="flex justify-center mt-4">
            <UniversalButton
              label="Save"
              onClick={handleSaveDynamicPdf}
            />
          </div>
        </Dialog>


        {/* {!isCarousal &&  first condition is only !isCarousel */}
        {(!selectedTemplateData?.type ||
          selectedTemplateData?.type !== "document" ||
          (selectedTemplateData?.type === "document" && pdfMode === "upload"))
          && !isCarousal &&
          (() => {
        ...
        {/* DOCUMENT PDF VARIABLES SECTION */}
        {/* {selectedTemplateData?.type === "document" &&
          pdfMode === "dynamic" &&
          selectedPdfTemp && (
            <div className="mt-2 space-y-4">
