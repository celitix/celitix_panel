...
// const Unsubscribe = ({ selectedWabaUser }) => {
const Unsubscribe = () => {
  ...
  const handleExport = async () => {
    ...
    if (!subData || subData.length === 0) {
      toast.error("No unsubscribe data available to export");
      return;
    }
    const payload = {
      wabaNumber: selectedWaba,
    ...
    try {
      const res = await exportUnsubscribeNumber(payload);
      if (!res?.success) {
        return toast.error(res?.msg);
      }
      triggerDownloadNotification();
      toast.success(res?.message || "File Downloaded Successfully");
    } catch (e) {
  ...
  return (
    <div
      className="relative p-4 bg-gray-100 rounded-xl border border-gray-100 h-full mb-2">
      {/* Header */}
      <div className="md:flex flex-wrap justify-between items-center hidden mb-2">
        ...
        <div className="flex gap-5">
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg px-3 py-2 text-center border">
            <p className="text-xl font-bold text-blue-500">{totalElements}</p>
            <p className="text-xs text-gray-600">Total Records</p>
          </div>
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg px-3 py-2 text-center border">
            <p className="text-xl font-bold text-blue-500">{subData.length}</p>
            <p className="text-xs text-gray-600">Records on this Page</p>
          </div>
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg px-3 py-2 text-center border">
      ...
      {/* Filter Section */}
      <div className="flex flex-wrap md:flex-nowrap items-end gap-3 mb-5 md:mb-4">
        <DropdownWithSearch
          id="selectWaba"
        ...
        <UniversalButton
          id="exportUnsubscribeBtn"
          name="exportUnsubscribeBtn"
          label="Export"
          icon={
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-80 lg:max-h-88 2xl:max-h-152 overflow-y-scroll pb-12"
          >
            {subData?.map((item) => (
              <motion.div
                key={item.srNo}
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="text-center py-16 bg-white rounded-t-md"
          >
            <img
              src="https://cdn-icons-png.flaticon.com/512/7486/7486759.png"
              alt="No data"
      ...
      {/* Pagination */}
      <div className="bottom-5 left-0 w-full px-4 py-2 rounded-b-md bg-white">
        <div className="flex justify-between items-center">
          {/* <span className="text-xs text-gray-500 font-semibold tracking-wide">
            Showing page {page} of {totalPages}
          </span> */}
          <span className="text-xs text-gray-500 font-semibold tracking-wide">
            Showing page {page} of {totalPages} ({totalElements} total)
          </span>
          <div className="flex gap-2">
            <button
              disabled={page === 1}
