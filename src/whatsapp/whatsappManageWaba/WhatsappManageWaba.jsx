...
const WhatsappManageWaba = ({ id, name }) => {
  ...
  console.log(wabadetails, "wabadetails...............")
  ...
  //   const wabaDetails = details.data;
  ...
  //   setDisplayName(
  //     wabaDetails?.name || row?.name || ""
  //   );
  // };

  const handleView = async (waba) => {
    setSelectedWaba(waba);
    setIsLoading(true); // Optional: show loader while fetching
    try {
      const details = await getwabadetails(waba.wabaNumber);
      // Access the first object in the data array
      const data = details?.data?.[0] || details?.data || null;
      setwabadetails(data);
      setView(true);
    } catch (error) {
  ...
  const handleEdit = async (row) => {
    ...
    try {
      // const details = await getwabadetails(row.wabaNumber);
      // const status = await getWabaDisplayNameStatus(row?.wabaSrno);

      const [details, statusRes] = await Promise.all([
        getwabadetails(row.wabaNumber),
        getWabaDisplayNameStatus(row.wabaSrno)
      ]);
      const wabaData = details?.data?.[0] || details?.data || {};
      console.log(statusRes)
      // Prefill all state variables for the Edit Dialog
      setAbout(wabaData.about || "");
      setDescription(wabaData.description || "");
      setAddress(wabaData.address || "");
      setEmail(wabaData.email || "");
      setVertical(wabaData.vertical || "");

      // Handle websites array (Prefilling the two inputs)
      if (wabaData.websites && Array.isArray(wabaData.websites)) {
        seteditWebsite1(wabaData.websites[0] || "");
        seteditWebsite2(wabaData.websites[1] || "");
      } else {
        seteditWebsite1("");
        seteditWebsite2("");
      }

      setPreview(wabaData.profile_picture_url || null);
      setDisplayName(wabaData.name || row.name || "");

      setWabaEdit(true); // Open dialog ONLY after data is set
    } catch (error) {
      toast.error("Error loading profile for editing");
    }
  };
  ...
  const rows = wabaList.map((waba, index) => ({
    ...
    additionalInfo: {
      ...
      isOnBizApp: waba.isOnBizApp === "1" ? "True" : "False" || "N/A",
      active: waba.active === "1" ? "active" : "inactive" || "N/A",
    },
    ...waba,
  }));
        ...
        >
          ...
          <div className="relative z-10 p-6">
            ...
            >
              ...
              <p>
                {wabadetails?.description || "No description provided."}
              </p>
            </motion.div>
