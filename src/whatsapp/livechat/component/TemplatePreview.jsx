      ...
      .map(({ type, text, buttons, format, example }, index) => {
        ...
        return (
          <div key={index} className="flex flex-col w-full gap-3">
            ...
            {type === "BODY" && (
              <pre className="border border-gray-200 rounded-md p-2 w-full bg-gray-50 text-[0.85rem] text-gray-800 overflow-auto min-h-20 max-h-60 break-words text-wrap">{text}</pre>
            )}
                      ...
                      <div className="flex flex-col gap-2 mt-1">
                        {buttons.map((btn, i) => (
                          <button
                            ...
                            className={`flex items-center justify-center border-b-2 border-[#128c7e] px-4 py-2 text-sm shadow-sm cursor-pointer rounded-xl w-full ${getBtnCss(
                              btn.type
                            )}`}
                            onClick={() => {
