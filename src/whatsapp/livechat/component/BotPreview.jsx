...
const BotPreview = ({ template }) => {
  ...
  const [tempDetails, setTemplateDetails] = React.useState(null);
  const [isFetching, setIsFetching] = React.useState(false);

  useEffect(() => {
    async function getTemplateDetails() {
      try {
        if (botJson?.type !== "template") return;
        setIsFetching(true);
        const allTemplates = await getWabaTemplateDetails(template?.wabaNumber);

        const desiredTemplate = allTemplates.find(
          (temp) => temp?.templateName === botJson?.template?.name
        );

        const tempDetails = await getTemplateDetialsById(
          desiredTemplate?.vendorTemplateId
        );
        setTemplateDetails(tempDetails?.components);
      } catch (e) {
  ...
  function renderFlow() {
    const flowName = botJson.interactive.body.text;
    const buttonName = botJson.interactive.action.name;

    return (
      <div className="bg-[#eee5dd] py-4 rounded-2xl w-[300px] flex flex-col items-center shadow-sm">
        {/* Flow Name */}
        <p className="text-sm font-semibold text-black mb-3">{flowName}</p>

        {/* Button */}
        <button
          className="w-[90%] flex items-center justify-center gap-2 py-2 rounded-full bg-white border-b-[3px] border-[#0a9385] text-[#0a9385] font-medium"
        >
          <FaReply className="text-[#0c8a7b]" />
          {buttonName}
        </button>
      </div>
    );
  ...
  function renderTemplate() {
    if (isFetching) {
      return (
        <div className="border border-gray-200 rounded-md w-90 p-3 bg-[#ece5dd]">
        ...
        {tempDetails?.map((item, index) => {
          if (item?.type === "HEADER" && item?.format === "IMAGE") {
            return (
              <img
                src={
                  botJson?.template?.components?.[0]?.parameters?.[0]?.image
                    ?.link
                }
                alt={
                  botJson?.template?.components?.[0]?.parameters?.[0]?.image
          ...
          if (item?.type === "HEADER" && item?.format === "VIDEO") {
            return (
              <video
                src={
                  botJson?.template?.components?.[0]?.parameters?.[0]?.video
                    ?.link
                }
                controls={true}
                autoPlay={false}
          ...
          if (item?.type === "HEADER" && item?.format === "DOCUMENT") {
            return (
              <iframe
                src={
                  botJson?.template?.components?.[0]?.parameters?.[0]?.document
                    ?.link
                }
                key={index}
          ...
          if (item?.type === "BODY") {
            return (
              <pre className="text-sm text-wrap " key={index}>
                {item?.text}
              </pre>
            );
          }
  ...
  return (
    <>
      ...
      {botJson?.type === "template" && renderTemplate()}
