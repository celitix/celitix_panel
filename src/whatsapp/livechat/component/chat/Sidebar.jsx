...
export const ChatSidebar = ({
  ...
  const filteredConvos =
    chatData?.allConversations?.filter(
      (convo) =>
        Array.isArray(switchChat) &&
        switchChat.some(
          (sw) =>
            sw?.mobile === convo?.mobileNo || sw?.mobileNo === convo?.mobileNo,
        ),
    ) || [];
  ...
  return (
    <div className="mt-2 pb-50 h-[100vh] max-h-full overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 ">
      {/* {JSON.stringify(wabaState, null, 2)}
      {"asdsa"} */}
      {selectedWabaUser === null && (
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
      ...
      {!isLoading && (
        <>
                ...
                return (
                  <motion.div
                    ...
                    className={`group p-4 rounded-xl cursor-pointer transition-all duration-200 mb-2 shadow-sm ${
                      chatState?.active?.mobileNo === chat.mobileNo
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex flex-col items-end justify-end">
                        ...
                        <p
                          className="text-xs transition-all text-nowrap"
                          title={moment(chat.insertTime).format(
          ...
          {selectedWabaUser?.mobileNo && filteredConvos.length === 0 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.3 }}
