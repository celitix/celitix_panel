...
import chatbot_header from "@/assets/animation/chatbot_header.json";
...
import FlowReplyPreview from "../FlowReplyPreview";
...
export const ChatScreen = ({
  ...
  const fetchSuggestedAiResponses = async () => {
    if (!aiInput?.msg?.trim()) {
      toast.warning("Please enter a message prompt");
      return;
    }

    const data = {
      wabaNumber: chatState?.active?.wabaNumber,
      mobileNo: chatState?.active?.mobileNo,
      // limit: Number(aiInput?.msgLimit) || 20,
      // limit: Number(aiInput?.msgLimit) || 20,
      message: aiInput?.msg.trim(),
    };

    try {
      setLoading(true); // ⏳ start loader

      const res = await suggestedAiResponse(data);

      if (res?.success) {
        setAiResponses(res);
        toast.success("AI responses generated successfully!");
      } else {
        toast.error(res?.message || "Failed to generate AI responses");
      }
    } catch (error) {
  ...
  // compute a key for the newest message
  const lastGroup =
    chatState?.specificConversation?.[
    chatState?.specificConversation?.length - 1
    ];
  const lastMsg = lastGroup?.messages?.[lastGroup.messages.length - 1];
  const lastMsgKey = lastGroup
    ? `${lastGroup.date}-${lastMsg?.id ?? "none"}`
    : "none";

  // a key that changes when you open/switch chats
  const initialKey =
    chatState?.active?.mobileNo || chatState?.active?.id || "none";
  useSmartScroll({
    containerRef: chatContainerRef,
    initialKey, // force scroll when opening/switching
    deps: [lastMsgKey, chatLoading], // scroll on new messages if near bottom
    bottomThreshold: 120,
  });
    ...
    <div className="relative flex flex-col flex-1 h-screen md:h-full">
      <div className="mt-0 md:mt-0 z-1 flex items-center bg-gray-50 justify-between w-full h-15 px-2 border rounded-tr-lg">
        ...
        <div className="flex items-center gap-2 justify-between">
          ...
          <CustomTooltip title="AI Input" placement="top" arrow>
            <button
            ...
            {chatState.specificConversation?.map((group, groupIndex) => (
              <div key={groupIndex} className="mb-5">
                <div className="flex items-center w-full">
                  <div className="border-b w-full border-dashed border-gray-400"></div>
                ...
                <div className="flex flex-col items-start space-y-2">
                  {group.messages.map((msg, index) => {
                    ...
                    const isFlowResponse = msg.replyType === "interactive" && msg.flowReplyJson;
                    // const isText = ["text", "button"].includes(msg.replyType);
                    const isText = ["text", "button"].includes(msg.replyType) && !isFlowResponse;
                      ...
                      >
                        {/* {isFlowResponse && (
                          <div className={`flex flex-col ${isSent ? "items-end" : "items-start"}`}>
                            <FlowReplyPreview jsonString={msg.flowReplyJson} />

                            <p className="text-[10px] text-gray-400 mt-1 italic">
                              User filled out the interactive flow
                            </p>
                          </div>
                        )} */}
                        ...
                        {isFlowResponse ? (
                          <div className={`flex flex-col ${isSent ? "items-end" : "items-start"}`}>
                            <FlowReplyPreview jsonString={msg.flowReplyJson} />
                            <p className="text-[10px] text-gray-400 mt-1 italic px-1">
                              User filled out the interactive flow
                            </p>
                          </div>
                        ) : (
                          <>
                                    ...
                                    <>
                                      {isImage && (
                                        ...
                                        >
                                          ...
                                          {msg?.caption && (
                                            <div
                                              className="text-sm text-gray-900 mb-1 ml-2 whitespace-pre-wrap break-words"
                                              dangerouslySetInnerHTML={{
                                                __html: formatMessageBody(
                                                  msg?.caption
                                                ),
                                              }}
                                            >
                            ...
                            {isBot && (
                              <div className="flex">
                                ...
                                <BotPreview template={msg} />

                                {btnOption === "active" && isSent && (
                                  <button className="flex justify-top">
                                    <Lottie
                                      animationData={chatbot_header}
                                      loop
                                      autoplay
      ...
      <Dialog
        header="🤖 AI Suggested Responses"
        visible={showAIInput}
        onHide={() => setShowAIInput(false)}
        className="md:w-[900px] w-full"
        draggable={false}
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">

          {/* LEFT – INPUT SECTION */}
          <div className="bg-white rounded-2xl shadow-sm border p-5 flex flex-col gap-4 transition-all hover:shadow-md">
            <h3 className="text-sm font-semibold text-gray-700">
              Prompt Settings
            </h3>

            <UniversalTextArea
              className="w-full p-3 rounded-xl border focus:ring-2 focus:ring-blue-400 transition"
              placeholder="Describe what responses you want..."
              value={aiInput.msg}
              row="10"
              onChange={(e) =>
                SetAiInput((prev) => ({
            ...
            {/* <InputField
              type="number"
              className="w-full p-3 rounded-xl border focus:ring-2 focus:ring-blue-400 transition"
              placeholder="Enter Last Chat conversation count (e.g. 3, 5, 10)"
              value={aiInput.msgLimit}
              onChange={(e) =>
                SetAiInput((prev) => ({
                  ...prev,
                  msgLimit: e.target.value,
                }))
              }
            /> */}
            ...
            <button
              disabled={loading}
              onClick={fetchSuggestedAiResponses}
              className={`
          ...
          ${loading
                  ...
                  : "bg-gradient-to-r from-blue-500 to-indigo-600 text-white hover:from-blue-600 hover:to-indigo-700 shadow-md hover:shadow-lg active:scale-[0.97]"
                }
        `}
            >
              {loading ? (
          ...
          <div className="relative bg-gradient-to-br from-gray-50 to-white border rounded-2xl p-5 h-full transition-all hover:shadow-md">

            {aiResponses?.message && (
              <p className="text-sm font-medium text-gray-700 text-center mb-4 animate-fadeIn">
                {aiResponses.message}
              </p>
            )}
            ...
            {aiResponses?.data ? (
              <div className="bg-white border border-gray-200 rounded-xl p-4 text-sm text-gray-700 space-y-3 break-words whitespace-pre-wrap animate-fadeIn">
                {Array.isArray(aiResponses.data) ? (
                  aiResponses.data.map((item, index) => (
                    <div
                      key={index}
                      className="p-3 rounded-lg bg-gray-50 border border-gray-100 transition hover:bg-gray-100"
                    >
                      {item}
                    </div>
                  ))
              ...
              !loading && (
                <div className="flex flex-col items-center justify-center text-center py-10 text-gray-500 animate-fadeIn">
                  <div className="mb-3 flex items-center justify-center h-14 w-14 rounded-full bg-blue-50 animate-bounce">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={1.8}
                        d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.77 9.77 0 01-4-.8L3 20l1.3-3.9A7.76 7.76 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"
                      />
                    </svg>
                  </div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Enter a prompt and click <span className="font-medium">Generate</span>
                  </p>
                </div>
              )
            )}
      ...
      <div className="md:block absolute bottom-0 md:bottom-0 w-full">
        ...
        {/* {isMessageSending ? <div>Sending a message</div> : ""} */}
        <AnimatePresence>
          {isMessageSending && (
            <motion.div
            ...
            >
              <div className="flex items-center gap-3 bg-white/80 backdrop-blur-sm border border-gray-200 shadow-sm px-4 py-2 rounded-2xl rounded-tr-none">
                <div className="flex gap-1">
                  <motion.span
                    animate={{ opacity: [0.4, 1, 0.4] }}
                    transition={{ repeat: Infinity, duration: 1.2, delay: 0 }}
                    className="w-1.5 h-1.5 bg-[#075e54] rounded-full"
                  />
                  <motion.span
                    animate={{ opacity: [0.4, 1, 0.4] }}
                ...
                <CircularProgress size={12} thickness={6} sx={{ color: '#075e54' }} />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <div className="flex-1 flex flex-col border-r pr-2">
            ...
            <div className="py-4">
              ...
              <div className="p-3 bg-gray-50 border rounded-md text-xs text-gray-700">
                Customer prefers evening calls. Interested in bulk WhatsApp
                campaigns.
              </div>
            </div>
