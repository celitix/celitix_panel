...
const WhatsappFlows = () => {
  ...
  const [openFlowsPreview, setOpenFlowsPreview] = useState(false);
  const [visible, setVisible] = useState(false);
  const [currentRow, setCurrentRow] = useState(null);
  const [flowId, setFlowId] = useState([]);
  const [flowPreviewData, setFlowPreviewData] = useState(null);
  ...
  const uniqueFlows = Array.from(
    new Map(
      (Array.isArray(flowList) ? flowList : []).map((flow) => [
        flow.flowId,
        flow,
      ])
    ).values()
  ...
  const filteredFlows = uniqueFlows.filter((flow) => {
    const searchText = search.toLowerCase();
    const flowName = (flow?.flowName || "").toLowerCase();
    const flowId = String(flow?.flowId || "");
    return flowName.includes(searchText) || flowId.includes(searchText);
  ...
  const handlePreview = async (flow = selectedFlow) => {
    setCurrentRow(selectedFlow);
    setOpenFlowsPreview(true);
    setIsFetching(true);
    ...
    try {
      ...
      // // Convert to pretty JSON string
      const jsonString = JSON.stringify(mainJsonObj, null, 2);
      setFlowPreviewData(mainJsonObj);
    } catch (error) {
      console.error("Error in exporting the data:", error);
    }
    setIsFetching(false);
  ...
  const getPreviewItems = (flowPreviewData) => {
    if (!flowPreviewData?.screens?.length) return [];

    const screen = flowPreviewData.screens[0];
    const children = screen?.layout?.children || [];

    // Case 1: Simple layout (TextHeading, Footer, etc.)
    if (children.length && !children[0]?.children) {
      return children;
    }

    // Case 2: Form layout (Form → children[])
    if (children[0]?.type === "Form" && Array.isArray(children[0].children)) {
  ...
  //   typeof flowPreviewData === "string"
  //     ? JSON.parse(flowPreviewData)
  //     : flowPreviewData
  // );

  console.log("flowPreviewData flow", flowPreviewData);
  ...
  const parsedFlowPreviewData = useMemo(() => {
    try {
      return typeof flowPreviewData === "string"
        ? JSON.parse(flowPreviewData)
        : flowPreviewData;
    } catch {
      return null;
  ...
  const screens = parsedFlowPreviewData?.screens || [];
  const activeScreen = screens[activeScreenIndex] || null;

  const previewItems = activeScreen
    ? getPreviewItems({
    ...
    <>
      <div className=" bg-white border border-gray-300 rounded-xl shadow-sm mb-3 md:px-3 px-0 pt-2">
        <div className="flex flex-col sm:flex-row sm:justify-between items-center mb-4">
          <div className="flex items-center gap-2 text-xl font-semibold mb-2 sm:mb-0 playf">
            Interactiv Flow Templates
            <LuWorkflow className="text-[#25D366] text-2xl" />
          </div>
      ...
      <div className="bg-white border border-gray-300 rounded-xl shadow-sm  md:p-4 pb-2 h-auto flex flex-col ">
        <div className="p-2  ">
          ...
          {/* Flows */}
          <div className="space-y-4 h-110 overflow-y-auto lg:overflow-y-hidden">
            {isLoading ? (
              ...
              paginatedFlows.map((flow) => (
                <div
                  // key={index}
                  key={`${flow.flowId}-${flow.status}-${flow.flowName}`}
          ...
          <Dialog
            visible={openFlowsPreview}
            onHide={() => setOpenFlowsPreview(false)}
            draggable={false}
            className={` ${screens.length > 1 ? "w-[40rem]" : "w-[30rem]"}`}
            header={currentRow?.flowName || "Flow Preview"}
          >
            <div className="w-full flex items-center justify-center gap-4">
              {screens.length > 1 && (
                <div className="flex items-center justify-between w-24 bg-white p-1 rounded-lg shadow-sm border-b-2 border-[#25D366]">
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    disabled={activeScreenIndex === 0}
                    onClick={() =>
                      setActiveScreenIndex((i) => Math.max(i - 1, 0))
                    }
                    className="flex items-center gap-1 text-[#075E54] font-bold py-2 px-4 rounded-full hover:bg-gray-100 disabled:opacity-30 transition-all text-xs"
                  >
                    <GrFormPrevious />
                    <span>BACK</span>
                  </motion.button>
                  ...
                  <motion.button
                    ...
                    onClick={() => setActiveScreenIndex((i) => Math.min(i + 1, screens.length - 1))}
                    className="flex items-center gap-1 bg-[#25D366] text-white font-bold py-2 px-5 rounded-full shadow-md disabled:bg-gray-300 disabled:shadow-none transition-all text-xs"
                  >
                    <span>NEXT</span>
              ...
              <div className="w-full overflow-hidden relative">
                <motion.div
                  key={activeScreenIndex} // Trigger animation on screen change
                  ...
                  {activeScreen && (
                    <PreviewMobilePanel
                      ...
                      onUpdateItem={(index, updater) => {
                        setFlowPreviewData((prev) => {
                          const data =
                            typeof prev === "string" ? JSON.parse(prev) : prev;

                          if (!data?.screens?.[activeScreenIndex]) return prev;

                          const screen = data.screens[activeScreenIndex];
                          const children = screen.layout.children;

                          let updatedLayoutChildren;
                          if (!children[0]?.children) {
                            updatedLayoutChildren = children.map((child, i) =>
                              i === index ? updater(child) : child
                            );
                          } else {
                            updatedLayoutChildren = [
                              {
                                ...children[0],
                                children: children[0].children.map((child, i) =>
                                  i === index ? updater(child) : child
                                ),
                              },
                            ];
                          ...
                          const updatedScreens = [...data.screens];
                          updatedScreens[activeScreenIndex] = {
                            ...screen,
                            layout: {
                              ...screen.layout,
                              children: updatedLayoutChildren,
                            },
                          };
