...
import LanguageSelect from "@/whatsapp/components/LanguageSelect";
import { TemplatePreview } from "@/whatsapp/livechat/component/TemplatePreview";
import AuthPreview from "@/whatsapp/whatsappcreatetemplate/components/AuthPreview";
import CarouselInteractiveActions from "@/whatsapp/whatsappcreatetemplate/components/CarouselInteractiveActions";
import CarouselTemplatePreview from "@/whatsapp/whatsappcreatetemplate/components/CarouselTemplatePreview";
import CarouselTemplateTypes from "@/whatsapp/whatsappcreatetemplate/components/CarouselTemplateTypes";
import InteractiveActions from "@/whatsapp/whatsappcreatetemplate/components/InteractiveActions";
import TemplateTypes from "@/whatsapp/whatsappcreatetemplate/components/TemplateTypes";
...
export const EditTemplate = () => {
    ...
    const [templateHeader, setTemplateHeader] = useState(state.header || "");
    const [templateFormat, setTemplateFormat] = useState(state?.body || "");
    ...
    useEffect(() => {
        const btns = state?.buttons?.find((item) => item?.type === "URL");
        const variables = state?.body_params;
        setInteractiveAction("callToActions");
        setUrl(btns?.url);
        setUrlTitle(btns?.text);

        const requiredVariable = variables?.map((item, index) => {
            return {
                id: (index + 1).toString(),
                value: item,
            };
