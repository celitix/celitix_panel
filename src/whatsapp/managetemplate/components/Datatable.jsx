  ...
  switch (rating?.toUpperCase()) {
    case "GREEN":
      return {
        ...
        label: "High Quality",
        description:
          "GREEN — High quality. The template has received little to no negative feedback from WhatsApp users.",
      };
    case "YELLOW":
      return {
        color: "bg-yellow-500/70 text-black",
        label: "Medium Quality",
        description:
          "YELLOW — Medium quality. The template has received some negative feedback or low read-rates.",
      };
    case "RED":
      return {
        ...
        label: "Low Quality",
        description:
          "RED — Low quality. The template has received significant negative feedback or low engagement.",
      };
    default:
      return {
        color: "bg-gray-400/70 text-white",
        label: "Unknown",
        description:
          "UNKNOWN — The quality score is pending. It will update once enough feedback is collected.",
      };
  }
};

// Capsule + Tooltip component
const QualityBadge = ({ rating }) => {
  const { color, label, description } = getQualityProps(rating);
  ...
  return (
    <>
      ...
      >
        <motion.div
          className={`px-3 py-1 rounded-full text-xs font-semibold shadow-sm ${color}`}
      ...
      {hovered &&
        ReactDOM.createPortal(
          <motion.div
            ...
            className="bg-gray-500 text-gray-100 text-xs rounded-lg px-3 py-2 shadow-lg pointer-events-none text-wrap w-60 text-center"
          >
            {description}
            <div className="absolute left-1/2 -bottom-1 w-2 h-2 rotate-45 bg-gray-500"></div>
          </motion.div>,
          document.body
        )}
    </>
...
const DataTable = ({
    ...
    try {
      ...
      await toast.promise(
        (async () => {
          ...
          await Promise.all([delay, refreshTemplate(id)]);
          await fetchTemplateData();
          return "Refreshed Successfully";
        })(),
        {
          loading: "Refreshing Template...",
      ...
      toast.error("Error refreshing template");
    } finally {
      setLoadingRow(null);
    }
  };
  ...
  const columns = [
    ...
    {
      field: "qualityScore", headerName: "Quality Score", flex: 1, minWidth: 120,
      renderCell: (params) => (
        <div className="flex items-center justify-center w-full h-full tracking-wide">
          <QualityBadge rating={params.value} />
        </div>
      ),
    },
      ...
      renderCell: (params) => (
        <>
          <CustomTooltip title="Refresh Status" placement="top" arrow>
            <IconButton
              className="text-xs"
              onClick={() => handleRefreshStatus(params.row)}
              ...
              <>
                ...
                {selectedRow?.templateData?.components?.some(
                  (comp) => comp.type === "BODY"
                    ...
                    <div className="contentbox text-sm text-gray-700 py-2 max-h-80 overflow-y-auto whitespace-pre-wrap">
                      {selectedRow?.templateData?.components
                        ?.filter((comp) => comp.type === "BODY")
                        .map((comp, index) => (
                          <p key={index} className="leading-relaxed"
                            dangerouslySetInnerHTML={{
                              __html: formatMessageBody(comp.text),
                            }}
                          >
                            {/* {comp.text} */}
                          </p>
                    ...
                    selectedRow?.templateData?.components
                      ...
                      .buttons?.map((btn, index) => (
                        <button
                          ...
                          className={`flex items-center justify-center border-b-2 border-[#128c7e] px-4 py-2 text-sm shadow-sm cursor-pointer rounded-xl w-full   mt-2  bg-white  ${getBtnCss(
                            btn.type
                          )}`}
                          onClick={() =>
                            btn.type === "PHONE_NUMBER"
