...
`.trim();
  ...
  const hasAIService = user?.services?.some(
    (service) => service.service_type_id === 11
  );

  return (
    <>
      <CustomTooltip
        title={hasAIService ? "Generate With AI" : "Service not available"}
        arrow
        placement="top"
      >
        <button
          onClick={() => hasAIService && setIsOpen(true)}
          disabled={!hasAIService}
          className={`absolute right-3 bottom-8 w-8 h-8 rounded-full flex items-center justify-center z-0
                    ${hasAIService
              ...
              <div className="flex gap-2">
                <div className="flex gap-1">
                  ...
                  <RadioButton
                    name="optimizeFor"
                    value="Click Rate"
                    checked={optimizeFor === "Click Rate"}
                ...
                <div className="flex gap-1">
                  ...
                  <RadioButton
                    name="optimizeFor"
                    value="Reply Rate"
                    checked={optimizeFor === "Reply Rate"}
                    onChange={(e) => setOptimizeFor(e.target.value)}
                  />
                </div>
              </div>
