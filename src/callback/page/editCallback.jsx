...
export const EditCallback = () => {
    ...
    useEffect(() => {
        ...
        setDetails((prev) => ({
          ...prev,
          callBackName: data?.row?.callbackName,
          authorizationType: authorizationType,
          callBackType: data?.row?.callbackType,
          allowCallBackDlr: String(data?.row?.allowCallbackDlr),
          token: data?.row?.token,
          userId: userId,
          password: data?.row?.password,

          callBackDlrUrl: url || rcsUrl || callbackUrl || callbackDlrUrl,
          url: url,
    ...
    async function handleUpdateCallback() {
        try {
            ...
            const srno =
              data?.rcsCallbackSrno || data?.whatsappCallbackSrno || data?.row?.srno;
            let payload = {
                srno: srno,
                ...details,
            ...
            if (details?.callBackType == "rcs") {
                delete payload.callBackDlrUrl;
                payload = {
                    ...payload,
                    rcsUrl: details?.callBackDlrUrl,
                ...
                payload = {
                    ...payload,
                    url: details?.callBackDlrUrl,
                    wabaNumber: details?.wabaNumber,
                };
            }
            // callBackType: rcsUrl ? "rcs" : "whatsapp",
    ...
    const fetchWabaList = async () => {
        try {
            ...
            if (response?.length) {
                ...
                setDetails((prev) => ({
                  ...prev,
                  wabaNumber: data?.res?.wabaNumber,
                }));
            } else {
    ...
    useEffect(() => {
      if (!details?.callBackType) return;
      if (details.callBackType == "rcs") {
