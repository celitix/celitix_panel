...
const Recharge = () => {
    ...
    const handleRefresh = async () => {
        setIsFetching(true);
        await Promise.all([fetchUsersBalance(), fetchAllUsersDetails()]);
        setIsFetching(false);
        toast.success("Data refreshed");
    };

    // Merge users + balances
    const mergedData = useMemo(() => {
        return balances
            .map((bal) => {
                const userMatch = allUsers.find((u) => u.srNo === bal.userSrNo);
                return {
                    userName: userMatch?.userName || "Unknown User",
                    srNo: bal.userSrNo,
    ...
    }, [balances, allUsers, search]);

    const totalBalance = mergedData.reduce((sum, u) => sum + parseFloat(u.balance || 0), 0);
    ...
    return (
        <>
            <div className="flex flex-wrap gap-4 bg-white p-5 rounded-xl">
                <div className="w-full py-5 text-center">
                    <div className="w-full py-2 lg:py-5 flex items-center justify-between">
                        <div></div>
                        <h1 className="text-xl font-semibold text-gray-700">
                            Account Recharge Portal
                        </h1>
        ...
        <div className="flex justify-center gap-4 mt-2">
          ...
          />
            ...
            >
                ...
                >
                    <div className="text-lg font-medium text-gray-800">
                        <MonetizationOn sx={{ mr: 1, verticalAlign: "middle", color: "green" }} />
                        User Balances
                    </div>
                    <div className="flex items-center gap-2">
            ...
            /> */}
                ...
                {/* Summary */}
                <div className="text-sm text-gray-700 my-2 font-medium text-left">
                    Total Users: <b>{mergedData.length}</b>

                    {/* | Total Balance:{" "}
          <b style={{ color: "green" }}>₹{totalBalance.toFixed(2)}</b> */}
                </div>
                ...
                <AnimatePresence mode="popLayout">
                    {isFetching ? (
                        ...
                        <div className="text-center flex items-center justify-center h-full text-gray-500 text-xl">
                            No users found.
                        </div>
                    ) : (
                        ...
                        <AccountCircle className="text-white" />
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-gray-500 text-xs">Sr No: {item.srNo}</span>
                                        </div>
                                    </div>
