...
const Transactions = () => {
  ...
  function handleExport() {
    ...
    const col = columns.map((c) => c.headerName || c.field);

    // Auto-detect fields, format numbers, and prevent scientific notation
    const fieldKeys = columns.map((c) => c.field);
    const row = rows.map((r) =>
      fieldKeys.map((field) => {
        ...
        // Detect phone numbers / long IDs → keep as text
        const isLongNumber = /^\d{10,15}$/.test(String(val));
        if (isLongNumber) return `'${val}`;

        // Convert numeric strings to numbers (for Excel sum)
  ...
  const walletusagecolumns = [
    ...
    { field: "recordDate", headerName: "Date", flex: 1, minWidth: 300 },
    { field: "walletUsage", headerName: "Closing Balance", flex: 1, minWidth: 300 },
  ];
