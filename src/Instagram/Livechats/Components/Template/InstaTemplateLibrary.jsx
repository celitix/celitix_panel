...
// APIS 
import { sendInstaMessage } from "@/apis/instagram/instagram.js";
...
const InstaTemplateLibrary = ({
  ...
  const handleSendTemplate = async (templateData, type) => {
    
  // Build local template state (optional, for UI only)
  const template = {
    title: templateData.title,
    subtitle: templateData.subtitle,
    imageUrl: templateData.imageUrl,
    fromMe: true,
    defaultAction: templateData.type && templateData.payload ? {
      type: templateData.type,
      url: templateData.payload,
    } : undefined,
    buttons: templateData.buttons || [],
  };

  try {
    // Dynamically build element (only include provided fields)
    const element = {};
    if (templateData.title) element.title = templateData.title;
    if (templateData.imageUrl) element.image_url = templateData.imageUrl;
    if (templateData.subtitle) element.subtitle = templateData.subtitle;

    if (templateData.defaultAction) {
      element.default_action = {
        type: templateData.defaultAction.type,
        url: templateData.defaultAction.payload,
      };
    }

    if (Array.isArray(templateData.buttons) && templateData.buttons.length > 0) {
      element.buttons = templateData.buttons.map((btn) => {
        if (btn.type === "web_url") {
          return {
            type: "web_url",
            ...(btn.url || btn.payload ? { url: btn.url || btn.payload } : {}),
    ...
    const res = await sendInstaMessage(payload);
    console.log("res send message", res);
  } catch (error) {
    console.error("Error sending Instagram template:", error);
  }
          ...
          >
            ...
            <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
              ...
              <button
                ...
                title="Send"
                onClick={() => {
                  handleSendTemplate(template, "generic");
                  if (onClose) onClose();
                }}
              >
            ...
            >
              ...
              <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                ...
                <button
                  ...
                  title="Send"
                  onClick={() => {
                    handleSendTemplate(temp, "button");
                    if (onClose) onClose();
                  }}
                >
                  <FiSend className="text-blue-700" />
