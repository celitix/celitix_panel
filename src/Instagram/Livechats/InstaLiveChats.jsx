...
const InstaLiveChats = ({
  ...
  const fetchInstaChats = async () => {
    try {
      const payload = {
        ...
        srno: 1,
        replyTime: "2025-09-07 23:59:59",
      };

      const res = await loadInstaChat(payload);
      console.log("res fetch insta chat", res)
    } catch (error) {
  ...
  const fetchAllConversations = async () => {
    try {
      ...
      console.log("res fetch all convo", res)
    } catch (error) {
      console.error("Error fetching Instagram chats:", error);
    }
  };

  const sendMessagetoInsta = async (data) => {
    try {
      const payload = {
  ...
  const viewInstaUpdate = async () => {
    try {
      ...
      console.log("res view insta update", res)
    } catch (error) {
      console.error("Error fetching Instagram chats:", error);
  ...
  const getInstaChatsOfOneUser = async () => {
    try {
      ...
      console.log("res get insta chat of one user", res)
    } catch (error) {
      console.error("Error fetching Instagram chats:", error);
    }
  };
        ...
        <div className="flex-1 relative h-full">
          ...
          {chatState.active && (
            <InstaChatScreen
              ...
              sendInstaMessage={sendInstaMessage}
            />
          )}
        </div>
      </div>
