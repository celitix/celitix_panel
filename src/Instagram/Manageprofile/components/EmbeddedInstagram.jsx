...
export default function EmbeddedInstagram({
  ...
  const REDIRECT_URI = "https://app.celitix.com/manageinstaprofile";
  const SCOPES = "user_profile,user_media"; // set scopes based on your needs
  const AUTH_URL = `https://www.instagram.com/oauth/authorize?force_reauth=true&client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}/&response_type=code&scope=instagram_business_basic%2Cinstagram_business_manage_messages%2Cinstagram_business_manage_comments%2Cinstagram_business_content_publish%2Cinstagram_business_manage_insights`;
  ...
  //       // console.log(
  //       //   "new URL(authWindow.location.href).searchParams",
  ...
  //           if (!res) {
  //             return toast.error("Error Onboard User");
  //           }
  //           authWindow.close();
  //           clearInterval(checkPopup);
  //         } else if (errorParam) {
  ...
  const handleConnect = () => {
    ...
    // Center on current screen (multi-monitor friendly)
    ...
    const handleSuccess = async (code) => {
      try {
        const res = await handleOnboardUser(code);
        if (!res) {
          toast.error?.('Error Onboard User');
          return;
        }
        toast.success?.('Connected! code send to backend successfully');
      } finally {
        cleanup();
      }
    };
  ...
  //     try {
  //       const res = await onboarduser(authCode);
  //       console.log("res", res);
  //     } catch (e) {
  //       console.log("e", e);
  //       toast.error("Error while onboard. Please try again later. ");
