...
export default function EmbeddedInstagram({
  userAceessToken,
  setUserAccessToken,
  setOnboaredStatus
}) {
  ...
  const REDIRECT_URI = "https://app.celitix.com";
  ...
  const handleConnect = () => {
    ...
    const vh =
      ...
      screen.height;
    const left = dualLeft + (vw - WIDTH) / 2;
    const top = dualTop + (vh - HEIGHT) / 2;
    ...
    const beginProcessing = () => {
      if (state !== "idle") return false; // already processing or done
      state = "processing";
      if (timerId) window.clearInterval(timerId); // stop polling immediately
      if (timeoutId) window.clearTimeout(timeoutId);
      return true;
    };

    // async function handleOnboardUser(code) {
    //   console.log("Sending code to backend:", code);
    //   try {
    //     const res = await onboarduser(code);
    ...
    //   } catch (e) {
    //     console.log("e", e);
    //     toast.error("Error while onboard. Please try again later. ");
    //     return false;
    //   }
    //   // finally {
    //   //   authWindow.close();
    ...
    //   response
    //   {
    //     "message": "The onboarding process has been completed successfully.",
    //     "data": null,
    //     "statusCode": 200,
    //     "jsonObject": null
    // }
    ...
    //   try {
    //     const res = await handleOnboardUser(code);
    //     if (res.statusCode !== 200) {
    ...
    //     toast.success(
    //       res?.message ||
    //       "The onboarding process has been completed successfully."
    //     );
    //   } catch (error) {
    //     console.error("Error during onboarding:", error);
    //     toast.error("Unexpected error while connecting user.");
    //   } finally {
    //     cleanup();
    //   }
    // };
    ...
    const handleSuccess = async (code) => {
      try {
        ...
        //   setOnboaredStatus(true);
        //   toast.success(
        //     res?.message || "The onboarding process has been completed successfully."
        //   );
        // } else {
        //   toast.error(res?.message || "Something went wrong while onboarding.");
        // }
        toast.success(
          res?.message || "The onboarding process has been completed successfully."
        );
        ...
        console.error("Error during onboarding:", error);
        toast.error("Error while onboarding. Please try again later.");
      } finally {
        cleanup();
      }
    };

    // Stop after a fixed time (prevents orphaned polling)
    timeoutId = window.setTimeout(() => {
      // toast.error?.('Login timed out.');
      cleanup();
    }, TIMEOUT_MS);

    // Poll until popup is closed or it reaches our redirect URI
    timerId = window.setInterval(async () => {
      try {
        // If user closed the popup manually
        if (authWindow.closed) return cleanup();

        // Only proceed after it’s navigated back to our redirect URI (same-origin)
        const href = authWindow.location?.href; // may throw until same-origin
        if (!href || href.indexOf(REDIRECT_URI) === -1) return;
        ...
        // If we’re here, we hit redirect_uri but neither code nor error — just clean up
        cleanup();
      } catch {
        // Cross-origin while on Instagram — safe to ignore until redirect happens
      }
    }, POLL_MS);
  };
