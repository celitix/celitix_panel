    ...
    <div className="relative p-6 md:p-6 rounded-4xl shadow-lg w-full flex flex-col bg-gradient-to-t from-indigo-100 via-purple-50 to-blue-100 h-full">
      ...
      {showCalendar && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.2 }}
          className="absolute right-0 top-0 bg-white p-6 sm:p-4 rounded-md shadow-lg z-50 flex md:flex-row flex-col gap-2"
        >
          <div className="w-54">
            <UniversalDatePicker
              id="walletUsage"
      ...
      <div className="h-100 overflow-hidden overflow-y-auto">
        ...
        <div className="divide-y text-sm h-full">
          ...
          {currentData.map((item, index) => (
            <div key={index} className="grid grid-cols-3 items-center py-2">
              <div className="">{(currentPage - 1) * rowsPerPage + index + 1}</div>
              <div>{moment(item.recordDate).format("DD MMM YYYY")}</div>
              <div className="font-medium text-gray-700">
                ₹ {item.walletUsage}
              </div>
      ...
      <div className="flex md:flex-row flex-col items-center md:items-end w-full border-t pt-2 mt-2 md:mt-0">
        <div className="text-sm font-medium text-gray-600 text-nowrap">
          Total Records : {walletUsageData.length}
        </div>
        {/* Pagination */}
        <div className="w-full">
          <div className="flex flex-wrap justify-center md:justify-end items-center mt-4 gap-2 sm:gap-0 ">
            <div className="flex gap-2 justify-center">
              {currentPage > 1 && (
                <button
                  onClick={() => goToPage(currentPage - 1)}
