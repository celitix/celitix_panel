...
// APIS
import { fetchAllUsers, fetchUserbySrno, fetchUserSrno, getUsersBalance, getSalesPersonList } from "@/apis/admin/admin";
import { getUserDetails } from "@/apis/user/user";
import { fetchIpDetails, fetchBalance, fetchTransactions } from "@/apis/settings/setting";
...
const SalesPersonDashboard = () => {
  ...
  // fetchAllUsers,fetchUserbySrno,getuserdetails,showipdetails,fetchUserSrno
  ...
  const fetchUserBySrno = async () => {
    try {
      ...
      console.log("res fetchUserBySrno", res)
    } catch (error) {
      console.log("error", error)
    }
  }
  ...
  const fetchUsersBalanceBySrno = async () => {
    try {
      ...
      console.log("res fetchUsersBalanceBySrno", res)
    } catch (error) {
      console.log("error", error)
    }
  }
  ...
  const fetchBalanceUser = async () => {
    try {
      ...
      console.log("res fetchBalanceUser", res)
    } catch (error) {
      console.log("error", error)
    }
  }
  ...
  const fetchSalesPersonList = async () => {
    try {
      ...
      console.log("res fetchSalesPersonList", res)
    } catch (error) {
      console.log("error", error)
    }
  }
  ...
  const stats = [
    {
      ...
      stripe:
        "bg-[repeating-linear-gradient(45deg,#fbcfe8_0_10px,#f9a8d4_0_20px)]",
    ...
    },
    {
      ...
      stripe:
        "bg-[repeating-linear-gradient(45deg,#bfdbfe_0_10px,#93c5fd_0_20px)]",
    ...
    },
    {
      ...
      stripe:
        "bg-[repeating-linear-gradient(45deg,#e9d5ff_0_10px,#d8b4fe_0_20px)]",
      changeColor: "text-green-500",
    },
    {
      title: "Conversions",
      ...
      stripe:
        "bg-[repeating-linear-gradient(45deg,#d1fae5_0_10px,#a7f3d0_0_20px)]",
      changeColor: "text-green-500",
    },
  ];
  ...
  return (
    ...
    //           {greeting}, Lingkan Roy
    //         </h3>
    //         <h1 className="text-4xl md:text-5xl font-extrabold mt-14 drop-shadow-lg">
    //           {time}
    //         </h1>
    ...
    //       <h4 className="mt-4 text-lg font-bold text-gray-800 relative z-10">
    //         Lingkan Roy
    //       </h4>
    //       <p className="text-sm text-gray-600 relative z-10">
    //         Product Designer
    //       </p>
    //       <p className="text-sm mt-1 text-gray-500 relative z-10">
    //         lingkan.roy@example.com
    //       </p>
    //     </motion.div>
    //   </div>
    ...
    //       >
    //         <div
    //           className={`absolute top-0 left-0 w-full h-full ${stat.stripe} opacity-20`}
    //         ></div>
    //         <h4 className={`text-sm text-${stat.color}-600 z-10 relative`}>
    //           {stat.title}
    //         </h4>
    //         <p
    //           className={`text-3xl font-bold text-${stat.color}-700 z-10 mt-2`}
    //         >
    //           {stat.value}
    //         </p>
    //         <span className={`${stat.changeColor} text-xs z-10 mt-1`}>
    //           {stat.change}
    //         </span>
    //       </motion.div>
    //     ))}
    //   </div>
    // </div>
    <div>Sales person dashboard</div>
  );
};
