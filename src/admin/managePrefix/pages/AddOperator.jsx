...
const AddOperator = ({ id, name }) => {
  ...
  async function handleFetchOpertorList() {
    ...
    if (!searchCountry) {
      toast.error("Please select a country to view operators.");
      return;
    }
    ...
    try {
      ...
      if (!res?.length) {
        setOperatorList([]);
        toast.error("No operators found for the selected country.");
        return;
      }
  ...
  const columns = [
    ...
    { field: "countryName", headerName: "Country", flex: 1, minWidth: 130 },
    { field: "operatorName", headerName: "Operator", flex: 1, minWidth: 130 },
    {
      field: "action",
      headerName: "Action",
      flex: 1,
  ...
  return (
    <div>
      <h1 className="text-xl text-center w-full text-gray-700">
        Manage Operator
      </h1>
      <div className="flex gap-2 items-end justify-between mb-3 w-full flex-wrap md:flex-nowrap">
