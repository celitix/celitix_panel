...
const ManageSMPPTable = ({ id, name, data, handleFetchSmppDetails }) => {
  ...
  const handleEdit = async (row) => {
    try {
      ...
      setEditDetails({
        ...res[0],
        // senderPrefix: res[0]?.prefixSenderId || "",
        messagePayload: res[0]?.messagePayload == "1" ? true : false,
        // tps: res[0]?.tps || "", // keep it as `tps` in state
      });

      setVersionEditStatus(res[0]?.Version);
      setTpEditStatus(res[0]?.tpsValue ? "enable" : "disable");
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(res?.message, "SMPP deleted successfully");
  ...
  const handleUpdateSmpp = async () => {
    ...
    try {
      ...
      let payload = {
        ...editDetails,
        // senderPrefix: editDetails?.senderPrefix,
      };

      // delete payload.prefixSenderId;
      // delete payload.tps; // remove backend field name
      payload.windowSize = payload.windowSize ?? "";

      if (tpeditStatus === "enable") {
        payload.tpsValue = editDetails?.tpsValue || "";
      } else {
        payload.tpsValue = "0"; 
      }

      if (versioneditStatus == 3.3) {
        // 3.3
        payload = { ...payload, Version: "3.3" };
        delete payload.messagePayload;
      } else if (versioneditStatus == 3.4) {
        // 3.4
        payload = {
          ...payload,
          Version: "3.4",
          messagePayload: editDetails.messagePayload ? "1" : "0" || "0",
        };
      }
      const res = await updateSMPP(payload);
      if (!res?.status) {
  ...
  const columns = [
    { field: "sn", headerName: "Sno", flex: 0, width: 70 },
    {
      field: "serviceName",
    ...
    { field: "webURL", headerName: "Web URL", flex: 1, minWidth: 120 },
    {
      field: "sendingPort",
    ...
    { field: "sockets", headerName: "Sockets", flex: 1, minWidth: 110 },
    {
      field: "connectivity",
      ...
      renderCell: (params) => (
        <div className="py-2">
          <CustomTooltip arrow title="Click to update status" placement="top">
            <div
              className={`text-white text-xs w-20 px-2 py-1.5 cursor-pointer border rounded-2xl text-center ${params.row.status ? "bg-green-500" : "bg-red-500"
                }`}
  ...
  return (
    <div>
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-2 items-center justify-between">
              ...
              {versioneditStatus === "3.4" && (
                <div className="">
                  ...
                  <Switch
                    checked={editDetails.messagePayload}
                    onChange={(e) => {
                      setEditDetails((prev) => ({
                        ...prev,
          ...
          <div className="grid md:grid-cols-4 grid-cols-1 gap-4">
            ...
            <InputField
              ...
              value={editDetails?.sendingPort}
              onChange={(e) =>
                setEditDetails({ ...editDetails, sendingPort: e.target.value })
              }
            />
            <InputField
              label="Receiver Port"
          ...
          <div className="grid md:grid-cols-4 grid-cols-1 gap-4">
            <AnimatedDropdown
              ...
              onChange={(e) =>
                setEditDetails({ ...editDetails, Registered: e })
              }
              value={editDetails?.Registered}
            />
            {/* <InputField
            ...
            <InputField
              ...
              name="senderidprefixedit"
              placeholder="Enter Sender Id Prefix"
              value={editDetails?.senderPrefix}
              onChange={(e) =>
                setEditDetails((prev) => ({
                  ...prev,
                  senderPrefix: e.target.value,
                }))
              }
            />
            <div className="flex flex-col">
          ...
          <div className="grid  md:grid-cols-4 grid-cols-1 gap-4">
            <InputField
              ...
              value={editDetails?.orderQueueSize}
              onChange={(e) =>
                setEditDetails({
                  ...editDetails,
            ...
            <InputField
              ...
              value={editDetails?.initialQueueSize}
              onChange={(e) =>
                setEditDetails({
                  ...editDetails,
            ...
            <InputField
              ...
              value={editDetails?.triggerQueueSize}
              onChange={(e) =>
                setEditDetails({
                  ...editDetails,
            ...
            <InputField
              ...
              value={editDetails?.ExpiryTime}
              onChange={(e) =>
                setEditDetails({ ...editDetails, ExpiryTime: e.target.value })
              }
            />
            {/* <AnimatedDropdown
              label="Window Size"
      ...
      {/* Edit SMPP Servie Edit */}

      {/* Delete SMPP Service Start */}
      <Dialog
