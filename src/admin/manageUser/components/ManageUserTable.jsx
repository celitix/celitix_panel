...
import {
  ...
  deleteHlrPricing,
  getHlrPricing,
  addSmsServicePricing,
  addEditEmailPricing,
  getEmailPricing,
  getEmailPricingByUser,
  deleteEmailPricing,
  addAiPricing,
  getUserAiPricing,
  getAiPricingList,
...
const ManageUserTable = ({ id, name, allUsers = [], fetchAllUsersDetails }) => {
  ...
  const [openVirtualEditDialog, setOpenVirtualEditDialog] = useState(false);
  const [virtualUserDetails, setVirtualUserDetails] = useState([]);

  // console.log("virtualUserDetails", virtualUserDetails);
    ...
    try {
      ...
      if (response?.userMstPojoList?.length > 0) {
        ...
        setUpdateDetails({
          ...
          userType: userDetails.userType?.toString() || "",
          ...
          // virtualBalance: userDetails.virtualBalance || "",
          virtualBalance: Number(userDetails.virtualBalance) || 0,
          accountManager: userDetails.salesPersonId || "",
  ...
  const handleNewEdit = async (srNo) => {
    try {
      const response = await fetchUserbySrno(srNo);
      if (response?.userMstPojoList?.length > 0) {
        const userDetails = response.userMstPojoList[0];
        setVirtualUserDetails(userDetails);
        setUpdateDetails({
          virtualBalance: userDetails.virtualBalance || "",
        });
        setSelectedId(srNo);
        setOpenVirtualEditDialog(true);
      } else {
        toast.error("No user details found for the selected user.");
      }
    } catch (error) {
  ...
  const handleDetailsUpdate = async () => {
    ...
    // const formattedExpiryDate = updateDetails?.expiryDate
    //   ? moment(updateDetails.expiryDate).format("YYYY-MM-DD")
    //   : null;

    let userType1 = updateDetails?.userType;
    let applicationType = "1";
    let accountUrl1 = "";

    if (updateDetails?.userType === "1") {
      // reseller
      userType1 = "1";
      applicationType = "2";
      accountUrl1 = accountUrl;
    } else if (updateDetails?.userType === "3") {
      // direct user
      userType1 = "3";
      applicationType = "1";
    } else if (updateDetails?.userType === "6") {
      userType1 = "6";
      applicationType = "1";
    } else if (updateDetails?.userType === "5") {
      userType1 = "5";
      applicationType = "1";
    }
    ...
    const data = {
      ...
      virtualBalance: updateDetails.virtualBalance,
      applicationType: applicationType,
      salesPersonId: updateDetails?.accountManager,
      creditLimit: updateDetails?.creditLimit,
    };
  ...
  const handleVirtualDetailsUpdate = async () => {
    const data = {
      userSrno: virtualUserDetails?.srno,
      virtualBalance: updateDetails.virtualBalance,
      creditLimit: updateDetails.creditLimit,
    };

    console.log("handleVirtualDetailsUpdate data", data);
    setEditDetailsLoading(true);

    try {
      const response = await updateVirtualBalance(data);
      if (response?.msg === "User Updated Successfully") {
        toast.success("User details updated successfully!");
        setEditDetailsDialogVisible(false);
        fetchAllUsersDetails();
        ...
        toast.success(response?.message || "Failed to update user details.");
      }
    } catch (error) {
      toast.error("Failed to update user details. Please try again.");
    } finally {
      setEditDetailsLoading(false);
      setOpenVirtualEditDialog(false);
    }
  };
  ...
  const handleSaveSmsPricing = async () => {
    ...
    if (res?.statusCode === 200) {
      ...
      const normalizedRows = d.map((item, index) => ({
        ...
        updateTime: item.update_time,
        transService: transOption?.find(
          (trans) => trans.value == item.transService
        )?.label || "-",
        promoService: promoOption?.find(
          (promo) => promo.value == item.promoService
        )?.label || "-",
      }));
  ...
  const handleSaveTransPromoService = async () => {
    // if (!trans) {
    //   toast.error("Please select transactional service.");
    //   return;
    // }
    ...
    const res = await addSmsServicePricing(payload);
    if (res?.success) {
      toast.success(res.message || "SMS Pricing saved successfully!");
      const d = await getSmsRateByUser(currentUserSrno);
      // resetSmsFields();
  ...
  function handleSMSEdit(d) {
    console.log("d.transService", d.transService);
    setTranscheck(!!d.transService);
    setPromocheck(!!d.promoService);
    setTrans(d?.transService?.toString() || "");
    setPromo(d?.promoService?.toString() || "");
    setSmsRate(d.rate || "");
    setDltRate(d.dltRate || "");
  }

  // const handleRemoveSMSPricing = async (data) => {
  //   try {
  //     const res = await deleteSmsRateByUser(currentUserSrno);

  //     if (!res?.status) {
  //       return toast.error("Error removing SMS pricing:");
  //     }
  ...
  //   } catch (e) {
  //     toast.error("Error removing SMS pricing:");
  //   }
  // };
  ...
  const smsCol = [
    ...
    { field: "updateTime", headerName: "Updated On", flex: 1, minWidth: 150 },
    {
      ...
      renderCell: (params) => (
        <>
          <CustomTooltip arrow title="Edit Rate" placement="top">
            <IconButton onClick={() => handleSMSEdit(params.row)}>
              <EditNoteIcon sx={{ fontSize: "1.2rem", color: "gray" }} />
            </IconButton>
          </CustomTooltip>
          <CustomTooltip arrow title="Delete Rate" placement="top">
  ...
  const [openLookupEdit, setOpenLookupEdit] = useState(false);
    ...
    try {
      ...
      if (res) {
        const formattedRow = {
          ...
          sn: 1,
          srno: res?.sr_no,
          numberLookupRate: res?.Rate,
          numberLookupInsertTime: res?.InsertTime,
        };

        setLookupRows([formattedRow]);
  ...
  const lookupColumns = [
    { field: "sn", headerName: "S.No", flex: 0, Width: 120 },
    { field: "numberLookupRate", headerName: "Rate (INR / Credit)", flex: 1 },
    { field: "numberLookupInsertTime", headerName: "Insert Time", flex: 1 },
    {
      field: "actions",
      headerName: "Actions",
      flex: 1,
      ...
      renderCell: (params) => (
        <>
          <CustomTooltip arrow title="Edit Rate" placement="top">
            <IconButton onClick={() => setOpenLookupEdit(true)}>
              <EditNoteIcon sx={{ fontSize: "1.2rem", color: "gray" }} />
            </IconButton>
          </CustomTooltip>
          <CustomTooltip arrow title="Delete Rate" placement="top">
            <IconButton onClick={() => setNumberLookupRateDeleteVisible(true)}>
              <DeleteForeverIcon sx={{ fontSize: "1.2rem", color: "red" }} />
            </IconButton>
          </CustomTooltip>
        </>
  ...
  // =======================================AI Pricing  RATE Start=======================================
  const [aiPricingRate, setAiPricingRate] = useState("");
  const [openAiPricingEdit, setAiPricingEdit] = useState(false);
  const [aiPricingRows, setAiPricingRows] = useState([]);
  const [aiPricingDeleteVisible, setAiPricingDeleteVisible] =
    useState(false);
  const [selectedAiPricing, setSelectedAiPricing] = useState(null);

  // SAVE Rate
  // async function handleAiPricingSave() {
  //   if (!aiPricingRate) {
  //     return toast.error("Please enter AI Pricing Token Rate");
  //   }
  //   try {
  //     const data = {
  //       userSrno: currentUserSrno,
  ...
  //     const res = await addAiPricing(data);

  //     if (!res?.msg?.includes("successfully")) {
  //       toast.error("AI Pricing saving failed");
  //     } else {
  //       toast.success("AI Token Rate update successfully");

  //       // setNumberLookupRate(res?.Rate);
  //       handleFetchAiPricing(currentUserSrno);
  //       setAiPricingEdit(false);
  //     }
  //   } catch (error) {
  //     toast.error("Failed to update AI Token Rate");
  //   }
  // }
  async function handleAiPricingSave() {
    if (!aiPricingRate) {
    ...
    try {
      const payload = {
        userSrNo: currentUserSrno,
        tokenRate: Number(aiPricingRate),
        srNo: selectedAiPricing?.srNo || 0, // 👈 IMPORTANT
      };

      const res = await addAiPricing(payload);

      if (!res?.success) {
        toast.error("AI Pricing save failed");
        return;
      }

      toast.success("AI Token Rate saved successfully");
      setAiPricingEdit(false);
      setSelectedAiPricing(null);
      handleFetchAiPricing(currentUserSrno);
    } catch (err) {
      toast.error("Failed to save AI Token Rate");
    }
  }


  // async function handleFetchAiPricing(userSrno) {
  //   try {
  //     const res = await getUserAiPricing({ srNo: 0, userSrNo: userSrno });
  ...
  //       setAiPricingRows([formattedRow]);

  //       // Update state fields
  //       setAiPricingRate(res?.Rate || "");
  //     }
  //   } catch (e) {
  //     console.error(e);
  //     toast.error("Failed to get AI Pricing Rate");
  //   }
  // }
  async function handleFetchAiPricing(userSrNo) {
    try {
      ...
      const userPricing = res.data.filter(
        (item) => item.userSrNo === userSrNo
      );

      const formattedRows = userPricing.map((item, index) => ({
        id: item.srNo,
        sn: index + 1,
        srNo: item.srNo,
        userSrNo: item.userSrNo,
        tokenRate: item.tokenRate,
      }));

      setAiPricingRows(formattedRows);

      // preload first rate if exists
      if (formattedRows.length) {
        setAiPricingRate(formattedRows[0].tokenRate);
      } else {
        setAiPricingRate("");
      }
    } catch (e) {
      console.error(e);
      toast.error("Failed to fetch AI Pricing");
    }
  }


  // Delete Rate
  // async function handleAiPricingRateDelete(userSrno) {
  //   try {
  //     const res = await deleteHlrPricing({ userSrno: currentUserSrno });
  //     if (!res?.msg?.includes("successfully")) {
  //       return toast.error(res?.msg || "AI Pricing Rate deletion failed");
  //     } else {
  //       setAiPricingDeleteVisible(false);
  //       toast.success("AI Pricing Rate deleted successfully");
  //       setAiPricingRate("");
  //       handleFetchAiPricing(currentUserSrno);
  //     }
  //   } catch (error) {
  //     toast.error("Failed to delete AI Pricing Rate");
  //   }
  // }
  async function handleAiPricingRateDelete() {
    if (!selectedAiPricing) return;

    try {
      const res = await getDeleteAiPricing({
        srno: selectedAiPricing.srNo,
        userSrno: selectedAiPricing.userSrNo,
      });

      if (!res?.success) {
        return toast.error(res?.msg || "Delete failed");
      }

      toast.success("AI Pricing deleted");
      setAiPricingDeleteVisible(false);
      setSelectedAiPricing(null);
      handleFetchAiPricing(currentUserSrno);
    } catch (e) {
      console.log(e)
      toast.error("Failed to delete AI Pricing");
    }
  }
  const aiPricingColumns = [
    { field: "sn", headerName: "S.No", width: 80 },
    {
      field: "tokenRate",
      headerName: "Rate (INR / Credit)",
      flex: 1,
    },
    {
      field: "actions",
      ...
      renderCell: (params) => (
        <>
          <CustomTooltip title="Edit Rate">
            <IconButton
              onClick={() => {
                setSelectedAiPricing(params.row);
                setAiPricingRate(params.row.tokenRate);
                setAiPricingEdit(true);
              }}
            >
              <EditNoteIcon sx={{ color: "gray" }} />
            </IconButton>
          </CustomTooltip>

          <CustomTooltip title="Delete Rate">
            <IconButton
              onClick={() => {
                setSelectedAiPricing(params.row);
                setAiPricingDeleteVisible(true);
              }}
            >
  ...
  const [mediaRate, setMediaRate] = useState(null);
  const [emailRate, setEmailRate] = useState("");
  const [emailRows, setEmailRows] = useState([]);
  const [pricingDetails, setPricingDetails] = useState([]);
  const [openEmailEditDialog, setOpenEmailEditDialog] = useState(false);
  const [editMediaRate, setEditMediaRate] = useState("");
  const [editEmailRate, setEditEmailRate] = useState("");
  const [editSrNo, setEditSrNo] = useState(null);
  const [openDeleteEmailDialog, setOpenDeleteEmailDialog] = useState(false);
  const [emailEditingRow, setEmailEditingRow] = useState(null);

  useEffect(() => {
    if (pricingDetails) {
      setEditMediaRate(pricingDetails.mediaRate || "");
      setEditEmailRate(pricingDetails.emailRate || "");
    }
  }, [pricingDetails]);

  const emailColumns = [
    { field: "sn", headerName: "S.No", flex: 0, minWidth: 80 },
    { field: "mediaRate", headerName: "Media Rate", flex: 1, minWidth: 120 },
    { field: "emailRate", headerName: "Email Rate", flex: 1, minWidth: 120 },
    ...
    { field: "updateTime", headerName: "Update Time", flex: 1, minWidth: 150 },
    {
      ...
      renderCell: (params) => (
        <>
          <CustomTooltip arrow title="Edit Pricing" placement="top">
            <IconButton onClick={() => handleEditEmailPricing(params.row.srno)}>
              <EditNoteIcon
                sx={{
                  fontSize: "1.2rem",
                  color: "gray",
          ...
          <CustomTooltip arrow title="Delete Pricing" placement="top">
            <IconButton
              onClick={() => handleDeleteEmailPricing(params.row.srno)}
            >
              <DeleteForeverIcon
                sx={{
                  fontSize: "1.2rem",
  ...
  const handleEmailPricing = async () => {
    const data = {
      srNo: 0,
      userSrNo: currentUserSrno,
      emailRate: Number(emailRate),
      mediaRate: Number(mediaRate),
    };
    try {
      const response = await addEditEmailPricing(data);
      if (response?.success === true) {
        toast.success(response?.message || "Email Pricing saved successfully");
        fetchEmailPricingByUser();
        setMediaRate("");
        setEmailRate("");
      }
    } catch (e) {
      console.log(e);
      toast.error("Error in saving the email pricing");
    }
  };

  const fetchEmailPricingByUser = async () => {
    try {
      const response = await getEmailPricingByUser(currentUserSrno);
      ...
      console.error(e);
      // toast.error("Error in fetching the email pricing by user");
    }
  };

  const handleEditEmailPricing = (srno) => {
    const row = emailRows.find((r) => r.srno === srno);
    if (!row) return;

    setEditSrNo(row.srno);
    setEditMediaRate(row.mediaRate);
    setEditEmailRate(row.emailRate);
    setOpenEmailEditDialog(true);
  };

  const handleEditPricingSave = async () => {
    const data = {
      srNo: editSrNo,
      userSrNo: currentUserSrno,
      emailRate: Number(editEmailRate),
      mediaRate: Number(editMediaRate),
    };
    ...
    try {
      ...
      if (response?.success === true) {
        toast.success(response?.message || "Email Pricing saved successfully");
        setOpenEmailEditDialog(false);
        setEditSrNo(null);
        await fetchEmailPricingByUser();
      }
    } catch (e) {
      console.log(e);
      toast.error("Error in saving the email pricing");
  ...
  const handleDeleteEmailPricing = (srno) => {
    const row = emailRows.find((r) => r.srno === srno);
    setEmailEditingRow(row);
    setOpenDeleteEmailDialog(true);
  };
  ...
  const handleAssign = async (srNo) => {
    ...
    const [
      ...
      getHlrPricing(srNo),
      getEmailPricingByUser(srNo),
      // getUserAiPricing({ srNo: "", userSrNo: srNo }),
      getAiPricingList(),
    ]);
    handleFetchAiPricing(srNo);

    if (hlrPricing) {
      const formattedRow = {
        id: hlrPricing?.sr_no || 1,
        sn: 1,
        srno: hlrPricing?.sr_no,
        numberLookupRate: hlrPricing?.Rate,
        numberLookupInsertTime: hlrPricing?.InsertTime,
      };
    ...
    if (aiPricing) {
      const formattedRow = {
        id: aiPricing?.sr_no || 1,
        sn: 1,
        srno: aiPricing?.sr_no,
        numberLookupRate: aiPricing?.Rate,
        numberLookupInsertTime: aiPricing?.InsertTime,
      };

      setAiPricingRows([formattedRow]);
      setAiPricingRate(aiPricing?.Rate);
    }

    setPricingDetails(emailPricing?.data);
    // Country List
    if (countryListRes) {
      setCountryOptions(
    ...
    const transOption = Array.isArray(transRes)
      ? transRes?.map((item) => ({
    ...
    const promoOption = Array.isArray(promoRes)
      ? promoRes?.map((item) => ({
        label: item.serviceName,
        value: String(item.serviceId),
      }))
    ...
    // Transaction
    setTransOptions(transOption);

    // Promotion
    setPromoOption(promoOption);

    const emailData = emailPricing?.data;
      ...
      setEmailRows([
        {
          ...
          srno: emailData.srNo,
          sn: 1,
          emailRate: emailData.emailRate,
          mediaRate: emailData.mediaRate,
          insertTime: emailData.insertTime,
          updateTime: emailData.updateTime,
        },
      ]);
    } else {
      setEmailRows([]);
    ...
    async function handleGetAllowedServices() {
      ...
      try {
        ...
        const formattedData = [
          ...
          { id: 11, name: "AI", enable: false },
          { id: 12, name: "INSTAGRAM", enable: false },
        ];
      ...
      renderCell: (params) => (
        <>
          ...
          {/* {params.row.role === "Reseller" && (
            <CustomTooltip arrow title="Edit Virtual Details" placement="top">
              <IconButton onClick={() => handleNewEdit(params.row.srno)}>
                <CiCreditCard2 sx={{ fontSize: "1.2rem", color: "gray" }} />
              </IconButton>
            </CustomTooltip>
          )} */}
  ...
  return (
    <>
      ...
      <Dialog
        ...
        className="lg:w-[75rem] md:w-[65rem] w-full"
        draggable={false}
      >
        <Box sx={{ width: "100%" }}>
          <Tabs
                  ...
                  <div className="w-full flex items-end gap-2">
                    ...
                    <div className="w-max-content">
                      <UniversalButton
                        ...
                        id="smsSave"
                        name="smsSave"
                        onClick={handleSaveTransPromoService}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-3 grid-cols-1 items-end gap-5 mt-2">
                  ...
                  <div className="w-full flex items-end gap-2">
                    <InputField
                      ...
                      // }
                      onChange={(e) => setDltRate(e.target.value)}
                      type="number"
                    />
                    <UniversalButton
                      label="Save"
          ...
          <CustomTabPanel value={value} index={4}>
            <>
              ...
              <div className="flex w-full flex-col gap-6 pb-5 md:m-3 lg:m-0">
                ...
                <div className="w-full">
                  <DataTable
                    height={288}
                    id="lookup-rate-table"
                    name="lookupRateTable"
                    col={lookupColumns}
                    rows={lookupRows}
                    getRowHeight={null}
                  />
              ...
              <Dialog
                header="Edit Lookup Rate"
                visible={openLookupEdit}
                style={{ width: "27rem" }}
                onHide={() => setOpenLookupEdit(false)}
                draggable={false}
              >
          ...
          <CustomTabPanel value={value} index={6}>
            <>
              {enableServices.find((s) => s.name == "AI")?.enable ? (
                <p className={badgeClasses.enabled}>Enabled</p>
              ) : (
                <p className={badgeClasses.disabled}>Disabled</p>
              )}
              <div className="flex w-full flex-col gap-6 pb-5 md:m-3 lg:m-0">
                ...
                <div className="flex flex-wrap md:flex-nowrap items-end gap-4">
                  <div className="w-full">
                    <InputField
                      label="Add Token Rate"
                      placeholder="(INR / Credit)"
                      type="number"
                      value={aiPricingRate}
                      onChange={(e) => setAiPricingRate(e.target.value)}
                    />
                  </div>
                  ...
                  <div className="w-full sm:w-auto">
                    <UniversalButton
                      label="Save"
                      id="updateAiPricingData"
                      name="updateAiPricingData"
                      onClick={handleAiPricingSave}
                    />
                  </div>
                </div>
                ...
                <div className="w-full">
                  <DataTable
                    height={288}
                    id="ai-pricing-table"
                    name="aipricingRateTable"
                    col={aiPricingColumns}
                    rows={aiPricingRows}
                    getRowHeight={null}
                  />
                </div>
              ...
              {/* Delete Number Lookup Rate */}
              <Dialog
                header="Delete AI Token Rate"
                visible={aiPricingDeleteVisible}
                style={{ width: "27rem" }}
                onHide={() => setAiPricingDeleteVisible(false)}
                draggable={false}
              >
                <div className="flex items-center justify-center">
                  <CancelOutlinedIcon sx={{ fontSize: 64, color: "#ff3f3f" }} />
                ...
                <div className="flex justify-center gap-4 mt-4">
                  <UniversalButton
                    label="Cancel"
                    onClick={() => setAiPricingDeleteVisible(false)}
                  />
                  <UniversalButton
                    label="Delete"
                    onClick={handleAiPricingRateDelete}
                  />
                </div>
              </Dialog>

              <Dialog
                header="Edit AI Token Rate"
                visible={openLookupEdit}
                style={{ width: "27rem" }}
                onHide={() => setOpenLookupEdit(false)}
                draggable={false}
          ...
          <CustomTabPanel value={value} index={5}>
            <>
              {enableServices.find((s) => s.name == "EMAIL")?.enable ? (
                <p className={badgeClasses.enabled}>Enabled</p>
              ) : (
                <p className={badgeClasses.disabled}>Disabled</p>
              )}
              <div className="flex flex-wrap items-end justify-start w-full gap-4 pb-5 align-middle lg:flex-nowrap">
                <InputField
                  ...
                  name="mediaRate"
                  label="Media Rate"
                  placeholder="Media Rate(INR / Credit)"
                  value={mediaRate}
                ...
                <InputField
                  ...
                  name="emailrate"
                  label="Email Rate"
                  placeholder="Email Rate(INR / Credit)"
                  value={emailRate}
                  onChange={(e) => setEmailRate(e.target.value)}
                  type="number"
                />
                <div className="flex justify-center mt-3">
                  <UniversalButton
              ...
              <Dialog
                header="Edit Email Pricing"
                visible={openEmailEditDialog}
                style={{ width: "48rem" }}
                onHide={() => setOpenEmailEditDialog(false)}
                draggable={false}
              >
                <div className="flex flex-wrap items-end justify-start w-full gap-4 pb-5 align-middle lg:flex-nowrap">
                  <InputField
                    ...
                    placeholder="Media Rate(INR / Credit)"
                    value={editMediaRate}
                    onChange={(e) => setEditMediaRate(e.target.value)}
                    type="number"
                  />
                  <InputField
                    id="emailrate"
                    ...
                    placeholder="Email Rate(INR / Credit)"
                    value={editEmailRate}
                    onChange={(e) => setEditEmailRate(e.target.value)}
                    type="number"
                  />
                  <div className="flex justify-center mt-3">
                    <UniversalButton
              ...
              <Dialog
                header="Delete Email Rate"
                visible={openDeleteEmailDialog}
                style={{ width: "27rem" }}
                onHide={() => setOpenDeleteEmailDialog(false)}
                draggable={false}
              >
                <div className="flex items-center justify-center">
                  <CancelOutlinedIcon sx={{ fontSize: 64, color: "#ff3f3f" }} />
                ...
                <div className="p-4 text-center">
                  <p className="text-[1.1rem] font-semibold text-gray-700">
                    Are you sure you want to delete the Email rate <br />
                  </p>
                  <p className="text-sm text-gray-500 mt-1">
                    This action cannot be undone.
                  </p>
                ...
                <div className="flex justify-center gap-4 mt-4">
                  <UniversalButton
                    label="Cancel"
                    onClick={() => setOpenDeleteEmailDialog(false)}
                  />
                  <UniversalButton
                    label="Delete"
                    onClick={async () => {
                      const res = await deleteEmailPricing({
                        userSrno: currentUserSrno,
                        srNo: emailEditingRow?.srno,
                      });
                      if (res?.message?.toLowerCase().includes("success")) {
                        toast.success("Rate deleted.");
                        fetchEmailPricingByUser();
                        setOpenDeleteEmailDialog(false);
                      } else {
                        toast.error(res.message || "Delete failed.");
                      }
                    }}
      ...
      <Dialog
        ...
        className="lg:w-[70rem] md:w-[40rem] w-[20rem]"
        draggable={false}
      >
        <div className="space-y-3">
            ...
            <DropdownWithSearch
              ...
              onChange={(e) => {
                setUpdateDetails({
                  ...updateDetails,
                  userType: e,
                  virtualBalance: e === "1" ? updateDetails.virtualBalance : 0,
                  creditLimit: e === "1" ? updateDetails.creditLimit : "",
                  domain: e === "1" ? updateDetails.domain : "",
                });
              }}
            />
            {updateDetails?.userType === "1" && (
              <>
                <div className="flex flex-col item-end gap-2">
                  ...
                  <div className="flex item-center gap-2 mt-1">
                    {/* <InputSwitch
                      ...
                      onChange={(e) => {
                        ...
                        setUpdateDetails({
                          ...updateDetails,
                          creditLimit:
                            value === 1 ? updateDetails?.creditLimit : "", // reset if disabled
                        });
                      }}
                      onLabel="Enabled"
                      offLabel="Disabled"
                    ...
                    /> */}
                    <InputSwitch
                      checked={updateDetails.virtualBalance === 1}
                      onChange={(e) => {
                        const value = e.value ? 1 : 0;
                        setUpdateDetails({
                          ...updateDetails,
                          virtualBalance: value,
                          creditLimit: value === 1 ? updateDetails.creditLimit : "",
                        });
                      }}
                      onLabel="Enabled"
                      offLabel="Disabled"
                      ...
                      offIcon="pi pi-times"
                    />
                    <label className="font-medium text-sm text-gray-600 mt-1">
                      {virtualBalance === 1 ? "Enable" : "Disable"}
                    </label>
                  </div>
                </div>
                {updateDetails.virtualBalance === 1 && (
                  <div className="w-full">
                    <InputField
                      label="Credit Limit"
                      id="creditLimit"
                      ...
                      value={updateDetails.creditLimit}
          ...
          <div className="flex justify-center mt-3">
            <UniversalButton
              label={editDetailsLoading ? "Updating User...." : "Update"}
      ...
      <Dialog
        header="Edit virtual details"
        visible={openVirtualEditDialog}
        onHide={() => setOpenVirtualEditDialog(false)}
        className="lg:w-[30rem] md:w-[30rem] w-[20rem]"
        draggable={false}
      >
        <div className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2 grid-cols-1">
            <div className="flex flex-col items-start gap-2">
              ...
              <div className="flex items-center gap-3 mt-1">
                <InputSwitch
                  ...
                  onChange={(e) => {
                    ...
                    setUpdateDetails((prev) => ({
                      ...prev,
                      virtualBalance: value,
                      creditLimit: value === 1 ? prev.creditLimit : "",
                    }));
                  }}
                  onLabel="Enabled"
                  offLabel="Disabled"
                ...
                />
                <label className="font-medium text-sm text-gray-600 mt-1">
                  {updateDetails.virtualBalance === 1 ? "Enabled" : "Disabled"}
                </label>
              </div>
            </div>
            ...
            {updateDetails.virtualBalance === 1 && (
              <div className="w-full">
                <InputField
                  ...
                  placeholder="Enter credit limit"
                  value={
                    updateDetails?.creditLimit ||
                    virtualUserDetails?.creditLimit ||
                    ""
                  }
                  onChange={(e) =>
                    setUpdateDetails((prev) => ({
          ...
          <div className="flex justify-center mt-4">
            <UniversalButton
              label={editDetailsLoading ? "Saving Balance..." : "Save"}
              disabled={editDetailsLoading}
              id="saveButton"
              name="saveButton"
              onClick={handleVirtualDetailsUpdate}
            />
          </div>
        </div>
      </Dialog>
