...
import { getAllWorkflow } from "@/apis/workflow";
...
const SendSms = () => {
  ...
  async function handleFetchWorkflow() {
    try {
      const res = await getAllWorkflow("sms");
  ...
  useEffect(() => {
    if (!Boolean(Number(workflowState.workflowFlag))) return;
    handleFetchWorkflow();
    ...
    <div className="mb-20 md:mb-0">
      ...
      >
        <ConfigureWorkflow
          ...
          headers={contactData?.fileHeaders}
          onClose={() => setWorkflowEditDialog({ isOpen: false, data: null })}
          setWorkflowState={(e) =>
