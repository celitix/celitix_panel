...
const CampaignDetailsReport = () => {
  ...
  const { campaignDetails, campaignName, total, data, selectedUser } = location.state || {};
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: 120,
      renderCell: (params) => (
        <div style={{ height: "100%", }} className="flex items-center md:justify-start justify-center" >
          <CustomTooltip title="Info" placement="top" arrow>
            ...
            >
              {clicked && Object.keys(clicked).length > 0 ? (
                <table className="w-50 md:w-90 text-sm text-left border border-gray-200 rounded-md overflow-hidden">
                  <tbody>
                    {[
