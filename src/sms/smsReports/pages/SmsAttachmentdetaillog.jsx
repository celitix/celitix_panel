...
const SmsAttachmentdetaillog = () => {
  ...
  const columns = [
    ...
    // },
    {
      ...
      width: 100,
      renderCell: (params) => (
        <div className="flex items-center md:justify-center justify-end h-full">
          ...
          >
            {data[params.row.id] ? (
              <div className="w-[250px] md:w-[320px] max-w-full mx-2 px-1">
                <div className="grid grid-cols-2 gap-y-2 text-sm text-gray-700">
                  {[
                    ...
                    <React.Fragment key={key}>
                      <div className="font-medium capitalize text-gray-600 border-b border-gray-200 pb-2 md:text-sm text-xs text-nowrap">
                        {label}
                      </div>
                      <div
                        className={`text-right font-semibold text-gray-800 border-b text-wrap border-gray-200 pb-2 md:text-sm text-xs ${key === "og_url"
  ...
  const handleFetchAttachmentDetails = async () => {
    try {
      setIsFetching(true);
      const res = await fetchDetailsAttachment(state.id, mobileNo, state.selectedUserId);
      setData(res);
    } catch (e) {
