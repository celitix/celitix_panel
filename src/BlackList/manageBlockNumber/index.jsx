...
import { FaUserAlt, FaBan } from "react-icons/fa";
...
const BlockNumber = () => {
  ...
  function handleDelete(row) {
    if (!row?.blockSrNo) return toast.error("Something went wrong");
    setDeleteData({
      isOpen: true,
  ...
  const totalServicesBlocked = rows.reduce(
    (acc, item) => acc + (item.blocked?.length || 0),
    0
  );
      ...
      >
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-3 my-4">
            {services?.map((item, index) => (
              <div
                className="flex items-center border gap-2 rounded-md p-2"
                key={item.name}
              >
                <input
                  type="checkbox"
