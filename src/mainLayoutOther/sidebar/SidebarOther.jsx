...
const SidebarOther = ({ isCollapsed, setIsCollapsed, isMobile }) => {
  const { user } = useUser();
  ...
  const menuItems = [
    ...
    },
    {
      ...
      links: [
        ...
        { to: `${prefix}/manageuser`, label: "Manage User" },
      ],
      roles: ["SALESPERSON", "ACCOUNTUSER"],
    },
    // {
    ...
    // },
    {
      ...
      type: "dropdown",
      links: [
        { to: `${prefix}/transactions`, label: "Transaction History" },
      ],
      roles: ["SALESPERSON", "ACCOUNTUSER"],
    },
  ...
  const getSidebarMenu = (role) => {
    return menuItems
      .filter((item) => !item.roles || item.roles.includes(role)) // filter main items
      .map((item) => {
        if (item.type === "dropdown") {
          return {
            ...item,
            links: item.links.filter((link) => !link.isHide), // hide links
          };
        }
        return item;
      });
  ...
  const filteredMenu = getSidebarMenu(user.role);
                            ...
                            {item.links.map((link) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className={`absolute inset-0 rounded-xl transition-all duration-200 ${isActive
                                    ? "bg-blue-50/80 border border-blue-100/50"
                                    : "group-hover:bg-gray-100/80"
                                    }`} />

                                  {/* Dot Indicator */}
                                  <div className={`relative w-1.5 h-1.5 rounded-full transition-all duration-300 mr-3 ${isActive ? "bg-blue-600 scale-125" : "bg-gray-300 group-hover:bg-gray-400"
                                    }`} />

                                  {/* Label */}
                                  <span className={`relative text-sm transition-colors duration-200 ${isActive ? "text-blue-700 font-semibold" : "text-gray-600 group-hover:text-gray-900 font-medium"
                                    }`}>
                                    {link.label}
                                  </span>

                                  {/* Right Arrow - Visible on Hover */}
                                  <div className={`relative ml-auto opacity-0 group-hover:opacity-100 transition-all duration-200 translate-x-1 group-hover:translate-x-0 ${isActive ? "text-blue-500 opacity-100" : "text-gray-300"
                                    }`}>
