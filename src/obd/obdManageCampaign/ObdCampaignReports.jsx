...
const ObdCampaignReports = () => {
  ...
  const handleDayWiseSummary = async () => {
    ...
    const data = {
      fromDate: moment(daywiseDataToFilter.toDate).format("YYYY-MM-DD"),
  ...
  const fetchCampaignReportsdata = async (data) => {
    try {
      ...
      const selectedType = campaignTypeOptions
        .find((opt) => opt.value === obdCampaignType)
        ?.label?.toLowerCase();
            ...
            <CustomTabPanel value={value} index={1}>
              <div className="w-full">
                <div className="flex flex-col md:flex-row lg:flex-row flex-wrap gap-4 items-end pb-5 w-full">
                  ...
                  <div className="w-full flex gap-2 sm:w-56">
                    <UniversalButton
