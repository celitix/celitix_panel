...
// Messenger Chat Bot UI
export function MessengerChatBot() {
  const BotFeature = ({ icon, text }) => (
    <div className="flex items-start gap-3 hover:bg-blue-50 p-3 rounded-lg transition border border-dashed">
      <div className="text-[#0084FF]">{icon}</div>
      <p className="text-xs text-gray-700">{text}</p>
    </div>
  );

  return (
    <div className="bg-gradient-to-br from-[#e8f0ff] via-white to-[#e3ecff] min-h-[85vh] flex items-center justify-center rounded-3xl overflow-scroll">
      <motion.div
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4 }}
        className="w-full max-w-md sm:max-w-lg md:max-w-xl bg-white rounded-3xl shadow-2xl p-4 sm:p-6 text-center"
      >
        {/* Bot Animation */}
        <div className="mb-5 flex items-center justify-center">
          <Lottie
        ...
        {/* Heading */}
        <h2 className="text-2xl sm:text-3xl font-bold text-[#0084FF] mb-2">
          Messenger Chat Bot
        </h2>

        <p className="text-xs text-gray-600 mb-5 px-3">
          Automate conversations, respond instantly, and engage customers on
          Messenger 24/7 using smart automation.
        </p>
        ...
        <div className="space-y-3 mb-6">
          <BotFeature
            icon={<ChatBubbleOutlineIcon />}
            text="Instant automated replies for incoming messages"
          />
          <BotFeature
            icon={<AccountCircleIcon />}
            text="Personalized responses based on customer behavior"
          />
          <BotFeature
            icon={<ShareIcon />}
            text="Send offers, links, and promotions automatically"
          />
        </div>

        {/* CTA */}
        <button
          className="w-full flex items-center justify-center gap-3 bg-[#0084FF] text-white font-semibold py-3 rounded-lg hover:bg-[#006ddf] transition-all shadow-md hover:shadow-lg"
        >
          <ChatBubbleOutlineIcon />
          Activate Chat Bot
        </button>
        ...
        <p className="text-[10px] text-gray-400 mt-3">
          * Bot behavior and responses can be customized in settings
        </p>
      </motion.div>
    </div>
  );
